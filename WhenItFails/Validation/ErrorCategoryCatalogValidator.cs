...
public sealed class ErrorCategoryCatalogValidator : IErrorCategoryCatalogValidator
{
   ...
   ErrorCatalogValidationResult result)
   {
      ...
      HashSet<string> allCategoryNames = new(StringComparer.OrdinalIgnoreCase);
      HashSet<string> usedAliases = new(StringComparer.OrdinalIgnoreCase);

      foreach(ErrorCategoryDefinition category in document.Categories)
      {
         ...
         string normalizedCategoryName = TextKeyNormalizer.NormalizeKey(category.Name);

         if(!string.IsNullOrWhiteSpace(normalizedCategoryName))
         {
            allCategoryNames.Add(normalizedCategoryName);
         }
      }
