...
## 2026-09-25 — isolated project-workspace initialization of precompiled 0.1.0 consumer (verification pending)
...
- Updated English `Docs/Published-Binary-Consumer-Smoke/en.md` and Toolroom usage/README. **Local execution pending**: the existing default and explicit bundled-default modes are previously confirmed PASS, but no project-workspace PASS is claimed until run. No production or xUnit test changes: complete `WhenItFails.Tests` remains **1445/1445 GREEN**. Next: run the optional project mode and record actual outcome; if successful, evaluate isolated malformed-project recovery scenario.
