...
## 2026-09-25 — pre-cancelled activation with unchanged precompiled 0.1.0 consumer (PASS confirmed)

- Added isolated opt-in `-ExerciseCancelledActivation` to `Toolroom/WhenItFails/PublicApiComparer/Test-PublishedConsumerBinary.ps1`. Before implementation, checked that the 0.1.0 initializer and built-in provider throw for an already cancelled token, while current runtime cancellation propagates before performing activation. The test does **not** claim to cover cancellation midway through catalog I/O.
- The disposable consumer is compiled once against exact requested NuGet `[0.1.0]`, then run unchanged against the package DLL and again after replacing only WhenItFails.dll with the source build, using independent disposable project workspace roots. It initializes a healthy project twice, then calls original `InitializeAsync(JsonsOptions, CancellationToken)` and `ResetToDefaultsAsync(CancellationToken)` with a **pre-cancelled** token. Both calls must propagate `OperationCanceledException` with the matching token.
- Both runs must retain reference-identical previously active context/status, non-degraded `ProjectCatalog` state, the existing error descriptor and unchanged SHA-256 of all five user-managed JSON files. The script also checks DLL load paths, unchanged consumer binary, source DLL hash and exact result parity. Existing seven smoke modes remain available.
- Updated English `Docs/Published-Binary-Consumer-Smoke/en.md` and Toolroom README/usage. **Locally confirmed by maintainer:** the eighth binary smoke returned `Binary cancelled activation smoke: PASS (original package consumer and swapped source DLL).` Both original and substituted DLL runs passed the pre-cancelled initialization/reset token, context/status identity, five-file integrity and descriptor checks. The run-specific report and DLL hashes were not supplied; NuGet publishing provenance remains unverified. Production code and xUnit tests unchanged; last confirmed full suite **1445/1445 GREEN**, eight binary smoke scenarios confirmed PASS. Next: add current-source-only deterministic queued-activation cancellation coverage; the historical 0.1.0 runtime has no instance-local activation gate, so do not assert queued cancellation semantics as a shared 0.1.0 contract. Cancellation during catalog I/O requires separate deterministic synchronization and cleanup tests.

## 2026-09-25 — queued activation cancellation behind opposing operation (1447/1447 GREEN)

- Added **two** focused cases to `WhenItFails.Tests/PublicApi/RuntimeActivationSerializationContractTests.cs` for the current default runtime: a queued reset cancelled while a project initializer holds the activation gate must never call its built-in provider; a queued project initialization cancelled while a reset holds the gate must never call its initializer. Deterministic task signals, rather than arbitrary sleeps, control both in-flight operations.
- The queued-reset case begins with an already valid published project and status. Cancellation must preserve its exact context, recorded status, store generation and completed activation sequence while the leading initializer remains blocked; only that successful leading initializer may later publish the replacement. The queued-initialization case must not publish any context before the leading reset finishes, and then records only that one completed reset. Both tests release their blockers in `finally`.
...
## 2026-09-25 — deterministic cancellation during catalog I/O and temporary-workspace cleanup (1449/1449 GREEN)
...
- **Verified locally by maintainer:** both additional focused tests and the complete **1449/1449 GREEN** suite. Eight binary smoke scenarios remain confirmed PASS. Compiler-warning count was not separately reported for this checkpoint. Next: harden bootstrap template creation so cancellation or failure during a new user-managed catalog write cannot leave a partial target that a later bootstrap would incorrectly treat as an existing catalog.

## 2026-09-25 — staged bootstrap template publication (1452/1452 GREEN)

- Hardened `JsonsBootstrapper` missing-template creation. A new project catalog is now written to a unique temporary file in the same target directory, cancellation is checked again after the asynchronous write, and only a fully written file is published with a non-overwriting `File.Move`. The temporary artifact is deleted best-effort in `finally`.
- If another writer creates the final target between the initial existence check and publication, the bootstrapper preserves the concurrent file and reports the template as already existing/skipped instead of overwriting it. This strengthens the existing "never overwrite project catalogs automatically" rule without changing the public constructor, interface, package version or persisted JSON schema.
- Added an **internal-only per-instance write seam** (available to the test assembly through the existing `InternalsVisibleTo`) so cancellation/failure can be injected deterministically without timing or huge-file tricks. `JsonsBootstrapperSafeTemplateWriteContractTests` adds three cases: cancellation after a staged write but before publish leaves no target/temp file; a concurrent target wins and is byte-preserved; and an injected I/O failure removes the staged file and returns the existing stable workspace I/O failure.
...
## 2026-09-25 — bootstrap re-entry after partial workspace (1456/1456 GREEN)
...
## 2026-09-25 — initializer/runtime recovery from a partial project workspace (1458/1458 GREEN)
...
## 2026-09-26 — malformed preserved catalog after partial workspace recovery (1460/1460 GREEN)
...
## 2026-09-26 — strict runtime retry after explicit repair of malformed project catalog (1462/1462 GREEN)
...
## 2026-09-26 — flexible fallback to project activation after partial-workspace repair (1463/1463 GREEN)
...
## 2026-09-26 — semantically inconsistent existing catalog during partial workspace recovery (1464/1464 GREEN)
...
## 2026-09-26 — late cancellation before initializer context publication (verification pending)

- Audited the default `ErrorCatalogInitializer.InitializeAsync` and found a real cancellation gap: it checked the token at entry and passed it to dependencies, but a bootstrapper/context provider could cancel the token and still return a successful `Task<Response<...>>`, allowing downstream work or a new context publication to proceed after cancellation.
- Production fix adds cooperative `ThrowIfCancellationRequested()` checks after each asynchronous dependency completes and immediately before synchronous store publication. No cancellation check is added *after* publication, to avoid reporting an activation as aborted after a write has already succeeded. No public API, package version or persisted JSON schema changes.
- Added `WhenItFails.Tests/Initialization/ErrorCatalogInitializerLateCancellationPublicationContractTests.cs` with two deterministic cases: a bootstrapper that cancels and then returns success must not invoke the context provider; a context provider that cancels and then returns success must not publish the candidate context. Both assert the exact cancellation token and preservation of the prior real `ErrorCatalogContextStore` publication record.
...
- **Verification pending:** 2 additional focused tests, expected complete suite **1467/1467 GREEN**. Last maintainer-confirmed complete suite **1465/1465 GREEN**; 8 binary smoke scenarios previously confirmed PASS. Next: run the focused late-cancellation publication class and complete suite; if the behavior is confirmed, record the 1467 checkpoint and examine cancellation at the runtime activation/status boundary without weakening publication ownership.
