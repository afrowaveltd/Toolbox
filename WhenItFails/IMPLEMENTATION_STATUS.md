...
## 2026-09-25 — matched completed status and detached combined catalog (verification pending)
...
- **Build correction pending local verification:** the first maintainer run stopped during compilation with CS7036 at `CompletedCombinedSnapshotContractTests.cs:66`: the `PreviousContextRecovery_AdvancesStatusSequenceWithoutChangingGeneration` test called its helper `CreateRuntime` without the required `builtIn` provider argument. Added an explicit `new FixedBuiltInProvider(Context("UNUSED-FALLBACK"))` in the test (commit `1d35823508fc62e8560ad74d1ce13f166c99e4f9`). No production code or public API was changed in the correction. **Focused and full tests have not been verified after the fix**; six new cases and expected full **1347/1347 GREEN** remain pending. Last maintainer-confirmed suite: **1341/1341 GREEN**. Published NuGet version and persisted catalog JSON schemas unchanged.
...
- Complete `WhenItFails.Tests` suite: **1341/1341 GREEN**, confirmed locally by maintainer after previous-context publication selection tests. The last explicitly confirmed warning-free build was at 1241/1241.
