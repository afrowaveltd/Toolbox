...
## 2026-09-25 — precompiled 0.1.0 consumer: bundled activation and descriptor probe (verification pending)
...
- Updated English `Docs/Published-Binary-Consumer-Smoke/en.md`, Toolroom README and usage docs. **Local opt-in execution pending:** do not mark this mode PASS based on the previously confirmed pre-initialization smoke. No xUnit or production changes; last maintainer-confirmed full suite **1445/1445 GREEN**. This opt-in probe is not a project-workspace `InitializeAsync()`, automatic recovery or comprehensive binary compatibility test. Next: run the opt-in script on Windows, record observed result, then consider isolated project workspace and recovery coverage.
