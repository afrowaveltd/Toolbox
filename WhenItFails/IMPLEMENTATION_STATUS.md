...
## 2026-09-25 — isolated first-start malformed project built-in fallback of precompiled 0.1.0 consumer (verification pending)
...
- Existing loaded-assembly, original executable hash, substituted DLL hash and exact result-parity checks remain active. Updated English `Docs/Published-Binary-Consumer-Smoke/en.md` and Toolroom README/usage. **Local first-start fallback run pending**; previous four smoke modes are maintainer-confirmed PASS. No production or xUnit test changes; complete `WhenItFails.Tests` remains **1445/1445 GREEN**. Next: run the new mode on Windows and record the actual result, then review strict-mode rejection behavior separately.
