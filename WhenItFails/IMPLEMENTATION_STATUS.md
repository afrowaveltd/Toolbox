...
## 2026-09-25 — precompiled 0.1.0 consumer strict first-start rejection (verification pending)
...
- Added English strict-mode procedure to `Docs/Published-Binary-Consumer-Smoke/en.md` and Toolroom usage/README. **Verification pending:** the five other binary smoke modes are maintainer-confirmed PASS, but this new strict first-start mode has not yet been run. No production or xUnit changes; confirmed suite remains **1445/1445 GREEN**. Next: execute opt-in strict first-start smoke, verify outcome and then test strict-mode reinitialization failure with a retained existing context separately.
