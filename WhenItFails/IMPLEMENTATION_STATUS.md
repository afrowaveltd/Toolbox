...
- **Build correction pending local verification:** the first maintainer run of `dotnet test WhenItFails.Tests --filter "FullyQualifiedName~PublishedCombinedSnapshotContractTests"` stopped before test execution with two CS7036 errors: `Response<T>.NotSupported` requires the `data` argument. Added `data: null` in `ErrorCatalogRuntime.GetCurrentPublication()` and `ErrorCatalogPublishedCombinedSnapshotExtensions.GetPublishedCombinedSnapshot()` (commits `a519d6d3fbfc8f43c905d1d1f8eb77f43cf43f59`, `dd9b365bdad84f651a6da71f5c279b002784be3a`). The correction changes neither the intended error status nor the snapshot contract. **Neither the focused nor the full suite has been verified after this fix.** Seven new tests and expected full **1300/1300 GREEN** remain pending; last confirmed suite **1293/1293 GREEN**. Existing runtime/store interface members, package version and persisted catalog JSON schemas are unchanged.
...
- Complete `WhenItFails.Tests` suite: **1293/1293 GREEN**, confirmed locally by maintainer after context publication identity tests. The last explicitly confirmed warning-free build was at 1241/1241.
