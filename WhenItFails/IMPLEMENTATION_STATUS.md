...
## 2026-09-25 — precompiled 0.1.0 consumer malformed project / previous-context recovery (verification pending)
...
- Added English recovery procedure to `Docs/Published-Binary-Consumer-Smoke/en.md` and updated Toolroom usage/README. **Verification pending:** do not mark the recovery mode PASS before a local run; earlier original, bundled-default and project-initialization modes are confirmed PASS. No production or xUnit changes; maintainer-confirmed full suite remains **1445/1445 GREEN**. Next: run the new opt-in recovery mode, inspect both executable outcomes and update this checkpoint with observed results.
