...
`ErrorCatalogFileName` and `CategoryCatalogFileName` are also rejected when their normalized final path segment is the current-directory marker, for example `Path.Combine("Nested", ".")`. These semantic-directory targets are invalid even when `Nested` does not yet exist, and they are rejected before workspace creation or template-provider invocation.
