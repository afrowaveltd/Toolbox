...
Targets that resolve outside the package directory are rejected. A target that resolves exactly to the package directory itself, a target ending with a directory separator, or a target that resolves to an already existing directory is rejected as an invalid target file name because template targets must identify files, not directories. The full provider snapshot is checked before file creation, so a later directory target cannot cause earlier templates to be written. Existing target files are still preserved and are never overwritten automatically.
