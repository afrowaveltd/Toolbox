...
All five caller-configured catalog file names must identify files rather than directory-only paths. `ErrorCatalogFileName`, `CategoryCatalogFileName`, `CodeGroupCatalogFileName`, `OwnerCatalogFileName`, and `ProfilesFileName` values ending with a directory separator are rejected as invalid configuration before the template provider is invoked or the workspace is created. All five caller-configured catalog file names are also rejected when their resolved paths already exist as directories; this is treated as invalid caller configuration before the template provider is invoked. Any of the five caller-configured catalog file names that resolves exactly to the package directory itself (for example, `"."`) is likewise classified as an invalid caller filename rather than a path outside the package. `ErrorCatalogFileName`, `CategoryCatalogFileName`, `CodeGroupCatalogFileName`, `OwnerCatalogFileName`, and `ProfilesFileName` are also rejected before provider invocation if any of their parent paths inside the package already exists as a regular file (for example, `Nested/errors.json`, `Nested/categories.json`, `Nested/code-groups.json`, `Nested/owners.json`, or `Nested/profiles.json` when `Nested` is a file). Actual escape paths retain the outside-package classification.
...
All five caller-configured catalog filename fields — `ErrorCatalogFileName`, `CategoryCatalogFileName`, `CodeGroupCatalogFileName`, `OwnerCatalogFileName`, and `ProfilesFileName` — are additionally rejected when their normalized final segment is the parent-directory marker while canonical resolution still remains inside the package, for example `Path.Combine("Nested", "Sub", "..")`. Such values resolve to a directory rather than a catalog file and are classified as invalid configuration before workspace creation or template-provider invocation. A true parent-directory escape outside the package retains the corresponding field-specific outside-package classification.
...
Targets that resolve outside the package directory are rejected. A target that resolves exactly to the package directory itself, ends with a directory separator, ends with a current-directory segment such as `Nested/.`, or ends with a parent-directory segment such as `Nested/Sub/..` while canonical resolution still remains inside the package is rejected as an invalid target file name because template targets must identify files, not directories. A true parent-directory escape outside the package retains the outside-package classification. A target that resolves to an already existing directory is likewise invalid. A nested target is also rejected if any of its parent paths inside the package already exists as a regular file, because that file cannot serve as a directory. The full provider snapshot must also contain unique canonical target paths: aliases such as `Nested/item.json` and `Nested/./item.json` are rejected with `WIF_JSONS_TEMPLATE_TARGET_FILE_NAME_DUPLICATE` before file creation. Snapshot targets must also be structurally compatible: one target may not identify a file at a path that another target requires as a directory, such as `Nested` together with `Nested/item.json`; these conflicts are rejected with `WIF_JSONS_TEMPLATE_TARGET_FILE_NAME_CONFLICT` before file creation. Path comparison follows platform semantics: case-insensitive on Windows and ordinal on non-Windows systems. The full provider snapshot is checked before file creation, so a later invalid directory, semantic-directory, file-parent, duplicate canonical target, or file/directory target conflict cannot cause earlier templates to be written. Existing target files are still preserved and are never overwritten automatically.
