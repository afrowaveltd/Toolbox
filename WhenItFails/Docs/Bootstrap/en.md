...
Caller-configured catalog file names must identify files rather than directory-only paths. For example, `ErrorCatalogFileName` and `CategoryCatalogFileName` values ending with a directory separator are rejected as invalid configuration before the template provider is invoked or the workspace is created.
