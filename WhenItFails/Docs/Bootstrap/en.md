...
`ErrorCatalogFileName` is also rejected when its normalized final path segment is the current-directory marker, for example `Path.Combine("Nested", ".")`. This semantic-directory target is invalid even when `Nested` does not yet exist, and it is rejected before workspace creation or template-provider invocation.
