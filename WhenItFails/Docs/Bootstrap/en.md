...
Targets that resolve outside the package directory are rejected. A target ending with a directory separator, or a target that resolves to an already existing directory, is also rejected because template targets must identify files, not directories. The full provider snapshot is checked before file creation, so a later directory target cannot cause earlier templates to be written. Existing target files are still preserved and are never overwritten automatically.
