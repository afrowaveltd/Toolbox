...
After maintainer-confirmed **1439/1439 GREEN**, the existing compiled-assembly inventory gained three tests for newer snapshot exports, internal capture-helper non-exposure, and deterministic complete type reporting. It emits its report from the locally compiled DLL when `AFROWAVE_WHENITFAILS_PUBLIC_API_REPORT` is set. `PublicApiComparer` additionally reports source/package exported-type totals and separate type/member differences against requested exact version `[0.1.0]`. The older 110-type and 611-entry counts are historical. The maintainer's latest comparer excerpt reports **148 source / 110 package exported types**, **830 source / 611 package API entries**, **38 source-only / 0 package-only types**, and **219 source-only / 0 package-only API entries**. The names/signatures of the source-only additions and complete inventory file have not yet been supplied. See [the current inventory procedure](../Current-Public-API-Inventory/en.md). The three new inventory tests await explicit maintainer confirmation (expected full **1442/1442 GREEN**); last explicitly confirmed full suite is **1439/1439 GREEN**.
