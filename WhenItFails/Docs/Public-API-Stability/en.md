...
## Detached supporting category projection (verification pending)

Added `ErrorCategoryCatalogSnapshot` (all 11 current category-document fields), `ErrorCategoryDefinitionSnapshot` (all eight current category-definition fields), and the additive `GetCategoryCatalogSnapshot(this IErrorCatalogRuntime)` extension. The snapshot types are sealed and getter-only; nested tags, category definitions, aliases, parent categories, default tags, mappings and metadata are copied into independent read-only collections. No live supporting document, definition or `MetadataBag` is exposed through the returned data. The extension takes a single context response and handles unavailable context, missing category catalog and ordinary capture failures without leaking source objects or exception detail. Six focused contract tests are committed in `ErrorCategoryCatalogSnapshotContractTests`, with expected full suite **1278/1278 GREEN** pending local verification. There is no change to the nine-method `IErrorCatalogRuntime` interface or the existing published 0.1.0 package.
