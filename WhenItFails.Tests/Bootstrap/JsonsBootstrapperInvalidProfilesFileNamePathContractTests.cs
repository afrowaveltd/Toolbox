...
public sealed class JsonsBootstrapperInvalidProfilesFileNamePathContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenProfilesFileNameContainsNullCharacter_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Response<JsonsBootstrapPayload> response =
                await bootstrapper.EnsureWorkspaceAsync(
                    new JsonsOptions
                    {
                        ...
                        ProfilesFileName = "profiles\0.en.json"
                    });
            ...
            Assert.Equal(
                "WIF_JSONS_PROFILE_CATALOG_FILE_NAME_INVALID",
                issue.Code);
            Assert.Equal(
                "The profile catalog file name is invalid.",
                response.Message);
            Assert.Equal(
                "The profile catalog file name is invalid.",
                issue.Message);
