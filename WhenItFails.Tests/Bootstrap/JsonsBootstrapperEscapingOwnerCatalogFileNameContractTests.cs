...
public sealed class JsonsBootstrapperEscapingOwnerCatalogFileNameContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenOwnerCatalogFileNameEscapesPackage_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Response<JsonsBootstrapPayload> response =
                await bootstrapper.EnsureWorkspaceAsync(
                    new JsonsOptions
                    {
                        ...
                        OwnerCatalogFileName = "../escaped.json"
                    });
            ...
            Assert.Equal(
                "WIF_JSONS_OWNER_CATALOG_FILE_NAME_OUTSIDE_PACKAGE",
                issue.Code);
            Assert.Equal(
                "The owner catalog file name must stay inside the package directory.",
                response.Message);
            Assert.Equal(
                "The owner catalog file name must stay inside the package directory.",
                issue.Message);
