...
public sealed class JsonsBootstrapperEscapingCategoryCatalogFileNameContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenCategoryCatalogFileNameEscapesPackage_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Response<JsonsBootstrapPayload> response =
                await bootstrapper.EnsureWorkspaceAsync(
                    new JsonsOptions
                    {
                        ...
                        CategoryCatalogFileName = "../escaped.json"
                    });
            ...
            Assert.Equal(
                "WIF_JSONS_CATEGORY_CATALOG_FILE_NAME_OUTSIDE_PACKAGE",
                issue.Code);
            Assert.Equal(
                "The category catalog file name must stay inside the package directory.",
                response.Message);
            Assert.Equal(
                "The category catalog file name must stay inside the package directory.",
                issue.Message);
