...
public sealed class JsonsBootstrapperEscapingProfilesFileNameContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenProfilesFileNameEscapesPackage_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                "WIF_JSONS_PROFILE_CATALOG_FILE_NAME_OUTSIDE_PACKAGE",
                issue.Code);
            Assert.Equal(
                "The profile catalog file name must stay inside the package directory.",
                response.Message);
            Assert.Equal(
                "The profile catalog file name must stay inside the package directory.",
                issue.Message);
