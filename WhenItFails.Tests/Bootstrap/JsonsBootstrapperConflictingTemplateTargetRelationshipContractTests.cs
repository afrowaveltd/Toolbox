...
public sealed class JsonsBootstrapperConflictingTemplateTargetRelationshipContractTests
{
    ...
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenChildTemplatePrecedesParentFileTarget_ReturnsInvalidWithoutPartialWrites()
    {
        string rootDirectory = Path.Combine(
            Path.GetTempPath(),
            "Afrowave",
        ...
        {
            JsonsBootstrapper bootstrapper = new(
                new ReverseConflictingTargetRelationshipTemplateProvider());
    ...
    private sealed class ReverseConflictingTargetRelationshipTemplateProvider
        : IJsonsTemplateProvider
    {
        public IReadOnlyList<JsonsTemplateFile> GetTemplateFiles(
            JsonsOptions options)
