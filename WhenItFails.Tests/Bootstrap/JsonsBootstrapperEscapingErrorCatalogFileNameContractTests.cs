...
public sealed class JsonsBootstrapperEscapingErrorCatalogFileNameContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenErrorCatalogFileNameEscapesPackage_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Response<JsonsBootstrapPayload> response =
                await bootstrapper.EnsureWorkspaceAsync(
                    new JsonsOptions
                    {
                        ...
                        ErrorCatalogFileName = "../escaped.json"
                    });
            ...
            Assert.Equal(
                "WIF_JSONS_ERROR_CATALOG_FILE_NAME_OUTSIDE_PACKAGE",
                issue.Code);
            Assert.Equal(
                "The error catalog file name must stay inside the package directory.",
                response.Message);
            Assert.Equal(
                "The error catalog file name must stay inside the package directory.",
                issue.Message);
