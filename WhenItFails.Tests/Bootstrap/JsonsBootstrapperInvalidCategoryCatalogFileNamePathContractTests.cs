...
public sealed class JsonsBootstrapperInvalidCategoryCatalogFileNamePathContractTests
{
    [Fact]
    public async Task EnsureWorkspaceAsync_WhenCategoryCatalogFileNameContainsNullCharacter_ReturnsInvalidBeforeProviderOrFilesystem()
    {
        ...
        try
        {
            ...
            Response<JsonsBootstrapPayload> response =
                await bootstrapper.EnsureWorkspaceAsync(
                    new JsonsOptions
                    {
                        ...
                        CategoryCatalogFileName = "categories\0.en.json"
                    });
            ...
            Assert.Equal(
                "WIF_JSONS_CATEGORY_CATALOG_FILE_NAME_INVALID",
                issue.Code);
            Assert.Equal(
                "The category catalog file name is invalid.",
                response.Message);
            Assert.Equal(
                "The category catalog file name is invalid.",
                issue.Message);
