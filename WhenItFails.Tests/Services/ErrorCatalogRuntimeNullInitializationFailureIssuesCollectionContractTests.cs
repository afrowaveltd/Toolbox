...
public sealed class ErrorCatalogRuntimeNullInitializationFailureIssuesCollectionContractTests
{
    [Fact]
    public async Task InitializeAsync_ShouldReturnStableFallbackFailure_WhenInitializationIssuesCollectionIsNull()
    {
        ErrorCatalogRuntime runtime = new(
            ...
            new FailingBuiltInContextProvider(),
    ...
    private sealed class NullIssuesFailingInitializer : IErrorCatalogInitializer
    {
        ...
        {
            Response<ErrorCatalogInitializationPayload> response = new()
            {
    ...
    private sealed class FailingBuiltInContextProvider
            ...
            CancellationToken cancellationToken = default)
        {
            return Task.FromResult(
                Response<ErrorCatalogContext>.Invalid(
                    code: "BuiltInCatalogUnavailable",
                    message: "The built-in catalog is unavailable."));
        }
    }
