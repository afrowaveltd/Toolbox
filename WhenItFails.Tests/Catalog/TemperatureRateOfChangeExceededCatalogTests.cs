...
public sealed class TemperatureRateOfChangeExceededCatalogTests
{
    [Fact]
    public async Task DefaultCatalogs_ShouldDefineTemperatureRateOfChangeExceededContract()
    {
        ...
        JsonElement error = FindByName(
            ...
            "TEMPERATURERATEOFCHANGEEXCEEDED");

        Assert.Equal("AFW_THM_0005", error.GetProperty("id").GetString());
        Assert.Equal(1_000_005, error.GetProperty("code").GetInt32());
        ...
        Assert.Equal(
            "when-it-fails/errors/thermal/temperature-rate-of-change-exceeded",
        ...
        Assert.Contains("RATE_OF_CHANGE", subcategories);
