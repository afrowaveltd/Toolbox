...
public sealed class ErrorCatalogContextProviderWhitespaceSourceMessageFallbackTests
{
    [Fact]
    public async Task LoadFromJsonsAsync_ShouldUseFallbackMessageAndPreserveSourceCode_WhenSourceMessageIsWhitespace()
    {
        const string sourceCode = "SourceErrorCatalogWhitespaceMessage";
        const string fallbackMessage = "Error catalog loading failed while creating catalog context.";

        ErrorCatalogContextProvider provider = new(
            new WhitespaceMessageInvalidErrorCatalogProvider(sourceCode),
    ...
    private sealed class WhitespaceMessageInvalidErrorCatalogProvider(string code)
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult(new Response<ErrorCatalogProviderPayload>
            {
                ...
                [
                    ...
                    {
                        Code = code,
                        Message = "This source issue message must not replace the fallback message."
                    }
                ]
            });
        }
