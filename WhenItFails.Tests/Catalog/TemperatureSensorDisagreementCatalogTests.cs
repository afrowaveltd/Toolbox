...
public sealed class TemperatureSensorDisagreementCatalogTests
{
    [Fact]
    public async Task DefaultCatalogs_ShouldDefineTemperatureSensorDisagreementContract()
    {
        ...
        JsonElement error = FindByName(
            ...
            "TEMPERATURESENSORDISAGREEMENT");

        Assert.Equal("AFW_THM_0006", error.GetProperty("id").GetString());
        Assert.Equal(1_000_006, error.GetProperty("code").GetInt32());
        ...
        Assert.Equal(
            "when-it-fails/errors/thermal/temperature-sensor-disagreement",
        ...
        Assert.Contains("REDUNDANCY", subcategories);
        ...
        Assert.Contains("REDUNDANCY", tags);
