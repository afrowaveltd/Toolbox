...
public sealed class ErrorCatalogContextProviderSuccessWithWarningsTests
{
    [Fact]
    public async Task LoadFromJsonsAsync_ShouldPreserveWarning_WhenErrorCatalogProviderSucceedsWithWarnings()
    {
        const string warningCode = "ErrorCatalogProviderWarning";
        const string warningMessage = "The error catalog loaded with a recoverable warning.";
        ...
        Assert.Equal(warningCode, outputWarning.Code);
        Assert.Equal(warningMessage, outputWarning.Message);
