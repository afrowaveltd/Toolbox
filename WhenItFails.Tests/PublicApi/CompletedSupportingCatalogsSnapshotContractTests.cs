...
public sealed class CompletedSupportingCatalogsSnapshotContractTests
{
    [Fact]
    public async Task CompletedCapture_UsesTheSelectedActivationStatusAndDetachesAllFourCatalogs()
    {
        ...
        Response<ErrorCatalogCompletedSupportingCatalogsSnapshot> response =
            runtime.GetCompletedSupportingCatalogsSnapshot();
        Assert.True(response.IsSuccess);
        ErrorCatalogCompletedSupportingCatalogsSnapshot observation =
            Assert.IsType<ErrorCatalogCompletedSupportingCatalogsSnapshot>(response.Data);
    ...
    public async Task PreviousContextRecovery_AdvancesActivationSequenceWithoutRepublishingCatalogs()
    {
        ...
        ErrorCatalogCompletedSupportingCatalogsSnapshot first =
            Assert.IsType<ErrorCatalogCompletedSupportingCatalogsSnapshot>(
                runtime.GetCompletedSupportingCatalogsSnapshot().Data);

        Assert.True((await runtime.InitializeAsync()).IsSuccess);
        ErrorCatalogCompletedSupportingCatalogsSnapshot recovered =
            Assert.IsType<ErrorCatalogCompletedSupportingCatalogsSnapshot>(
                runtime.GetCompletedSupportingCatalogsSnapshot().Data);
    ...
    public async Task RepublishingSameContextReference_RejectsStaleCompletedStatus()
    {
        ...
        Response<ErrorCatalogCompletedSupportingCatalogsSnapshot> result =
            runtime.GetCompletedSupportingCatalogsSnapshot();

        Assert.Equal(ResultStatus.Invalid, result.Status);
        Assert.Null(result.Data);
        Assert.Contains(result.Issues,
            issue => issue.Code == "WIF_COMPLETED_SUPPORTING_PUBLICATION_CHANGED");
    }
    ...
    public async Task RecoveryDuringCopy_WithSameGeneration_RejectsChangedStatus()
    {
        ...
        Assert.Null(result.Data);
        Assert.Contains(result.Issues,
            issue => issue.Code == "WIF_COMPLETED_SUPPORTING_STATUS_CHANGED");
    ...
    public void BeforeAnyCompletedActivation_ReturnsUnavailableWithoutInventingIdentity()
    {
        ...
        Assert.Null(result.Data);
        Assert.Contains(result.Issues,
            issue => issue.Code == "WIF_COMPLETED_SUPPORTING_UNAVAILABLE");
    }
    ...
    public void LegacyStore_ReturnsNotSupportedWithoutFallbackGeneration()
    {
        ...
        Assert.Null(result.Data);
        Assert.Contains(result.Issues,
            issue => issue.Code == "WIF_COMPLETED_SUPPORTING_NOT_SUPPORTED");
    }
    ...
    public async Task SecondPublicationReadOrdinaryException_IsNormalizedWithoutDetails()
    {
        ...
        Assert.Null(result.Data);
        Assert.Contains(result.Issues,
            issue => issue.Code == "WIF_COMPLETED_SUPPORTING_FAILED");
    ...
    {
        Assert.Contains(typeof(IErrorCatalogRuntimeSupportingObservationReader),
            typeof(ErrorCatalogRuntime).GetInterfaces());
        Assert.DoesNotContain(typeof(IErrorCatalogRuntime).GetMethods(),
            method => method.Name == "GetCompletedSupportingCatalogsSnapshot");
        Assert.DoesNotContain(typeof(IErrorCatalogRuntimeCombinedObservationReader)
            .GetMethods(), method => method.Name == "GetCompletedSupportingCatalogsSnapshot");

        Type type = typeof(ErrorCatalogCompletedSupportingCatalogsSnapshot);
