...
public sealed class WhenItFailsProfileWorkspaceEditorExcludedTagRemovalTests
{
    [Fact]
    public async Task ProfileRemoveExcludedTagAsync_WithExcludedTag_RemovesCanonicalValue()
    {
        ...
        string tag = await LoadFirstTagAsync(temporaryWorkspace.WhenItFailsJsonsPath);
        Response<ErrorProfileDefinition> addTagResponse =
            await editor.ProfileAddExcludedTagAsync(
                temporaryWorkspace.ProjectRootPath,
                "DITA_TEST",
                tag);
        Assert.True(addTagResponse.IsSuccess);
        ...
        Response<ErrorProfileDefinition> removeResponse =
            await editor.ProfileRemoveExcludedTagAsync(
                temporaryWorkspace.ProjectRootPath,
                " dita test ",
        ...
        Assert.DoesNotContain(
            ...
            removeResponse.Data.ExcludeTags);
    ...
    [Fact]
    public async Task ProfileRemoveExcludedTagAsync_WithTagNotExcluded_ReturnsNotFoundWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ...
        Assert.Contains(
            response.Issues,
            issue => issue.Code == "ProfileTagNotExcluded");
    ...
    [Fact]
    public async Task ProfileRemoveExcludedTagAsync_WithUnknownTag_ReturnsNotFoundWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    [Fact]
    public async Task ProfileRemoveExcludedTagAsync_WithUnknownProfile_ReturnsNotFound()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    public async Task ProfileRemoveExcludedTagAsync_WithEmptyValue_ReturnsInvalid(
        string profileName,
        string tagName)
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
    ...
    private static async Task<string> LoadFirstTagAsync(string whenItFailsJsonsPath)
    {
