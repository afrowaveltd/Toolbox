...
public sealed class WhenItFailsProfileWorkspaceEditorErrorTests
{
    ...
    public async Task ProfileAddErrorAsync_WithExistingError_AddsCanonicalId(
        Func<ErrorDefinition, string> lookupSelector)
    {
        ...
        Assert.Contains(canonicalErrorId, response.Data.IncludeErrors);

        ErrorProfileDefinition savedProfile = await LoadProfileAsync(
            temporaryWorkspace.WhenItFailsJsonsPath,
            "DITA_TEST");

        Assert.Contains(canonicalErrorId, savedProfile.IncludeErrors);
    ...
    [Fact]
    public async Task ProfileAddErrorAsync_WithDuplicate_ReturnsInvalidWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ...
        Assert.Contains(
            ...
            issue => issue.Code == "ProfileErrorAlreadyIncluded");
    ...
    [Fact]
    public async Task ProfileAddErrorAsync_WithUnknownError_ReturnsNotFoundWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    [Fact]
    public async Task ProfileAddErrorAsync_WithUnknownProfile_ReturnsNotFound()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    public async Task ProfileAddErrorAsync_WithEmptyValue_ReturnsInvalid(
        string profileName,
        string errorLookup)
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
