...
public sealed class WhenItFailsDocumentationKeySuggesterCaseInsensitiveSuffixCollisionTests
{
    [Fact]
    public async Task SuggestAsync_WhenUppercaseSecondSuffixExists_ReturnsThirdSuffix()
    {
        ...
        Response<DocumentationKeySuggestion> initialResponse = await suggester.SuggestAsync(
            options,
            "NETWORK",
            "Case insensitive suffix sample");
        ...
        errors.Add(CreateCollision(
            template,
            "AFW_TEST_9911",
            991001,
            "CASEINSENSITIVESUFFIXBASE",
            initialSuggestion.DocumentationKey));
        errors.Add(CreateCollision(
            template,
            "AFW_TEST_9912",
            991002,
            "CASEINSENSITIVESUFFIXTWO",
            $"{initialSuggestion.DocumentationKey}-2".ToUpperInvariant()));
        ...
        Assert.Equal($"{initialSuggestion.DocumentationKey}-3", suggestion.DocumentationKey);
    }
        ...
        string documentationKey)
    {
        ...
        collision["code"] = code;
        collision["name"] = name;
        collision["title"] = "Case insensitive suffix sample";
