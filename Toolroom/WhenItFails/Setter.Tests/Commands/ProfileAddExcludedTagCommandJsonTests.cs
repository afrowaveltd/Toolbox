...
public sealed class ProfileAddExcludedTagCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndPersistsExcludedTag()
    {
        ...
        string canonicalTag = TextKeyNormalizer.NormalizeKey(tagLookup);
        ...
        Assert.Equal("profile-add-excluded-tag", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(canonicalTag, data.GetProperty("addedExcludedTag").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.Contains(canonicalTag, saved.ExcludeTags, StringComparer.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task ExecuteAsync_WithJsonBeforeTag_AddsExcludedTag()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(
            ...
            document.RootElement.GetProperty("data").GetProperty("addedExcludedTag").GetString());
    }

    [Fact]
    public async Task ExecuteAsync_WithDuplicateExcludedTagAndJson_WritesStructuredFailureWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(JsonValueKind.Null, data.GetProperty("addedExcludedTag").ValueKind);
    ...
    public async Task ExecuteAsync_WithDuplicateJsonSwitch_ReturnsInputErrorWithoutBackup()
    {
        ...
        int exitCode = await ProfileAddExcludedTagCommand.ExecuteAsync(
        [
            "profile-add-excluded-tag",
            workspace.ProjectRootPath,
            profile.Name,
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await ProfileAddExcludedTagCommand.ExecuteAsync(args);
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_EXCLUDED_TAG_TEST",
                "JSON Excluded Tag Test");
