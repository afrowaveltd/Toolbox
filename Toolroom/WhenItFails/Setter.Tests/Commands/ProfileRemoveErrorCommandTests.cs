...
public sealed class ProfileRemoveErrorCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidArguments_RemovesIncludedError()
    {
    ...
    [Fact]
    public async Task ExecuteAsync_WithErrorNotIncluded_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    public static TheoryData<string[]> MissingArgumentCases =>
        new()
        {
            new[] { "profile-remove-error" },
            new[] { "profile-remove-error", "." },
            new[] { "profile-remove-error", ".", "DITA_TEST" }
        };
    ...
    public async Task ExecuteAsync_WithMissingRequiredArgument_ReturnsCommandInputError(
        string[] args)
    {
        Assert.Equal(1, await ProfileRemoveErrorCommand.ExecuteAsync(args));
    }
    ...
    public async Task ExecuteAsync_WithTooManyArguments_ReturnsCommandInputError()
    {
        Assert.Equal(
            1,
            await ProfileRemoveErrorCommand.ExecuteAsync(
                ["profile-remove-error", ".", "DITA_TEST", "AFW_GEN_0001", "Unexpected"]));
    }
