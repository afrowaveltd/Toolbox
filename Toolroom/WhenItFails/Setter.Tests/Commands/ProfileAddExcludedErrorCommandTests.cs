...
public sealed class ProfileAddExcludedErrorCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidArguments_AddsExcludedErrorAndReturnsSuccess()
    {
        ...
        int exitCode = await ProfileAddExcludedErrorCommand.ExecuteAsync([
            "profile-add-excluded-error", workspace.ProjectRootPath, " dita test ", error.Code.ToString()]);

        Assert.Equal(0, exitCode);
        ErrorProfileDefinition profile = await LoadProfileAsync(workspace.WhenItFailsJsonsPath, "DITA_TEST");
        Assert.Contains(TextKeyNormalizer.NormalizeKey(error.Id), profile.ExcludeErrors);
    }

    [Fact]
    public async Task ExecuteAsync_WithDuplicate_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace workspace = await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        WhenItFailsProfileWorkspaceEditor editor = new();
        Assert.True((await editor.AddProfileAsync(workspace.ProjectRootPath, "DITA_TEST", "DiTa Test")).IsSuccess);
        ...
        int exitCode = await ProfileAddExcludedErrorCommand.ExecuteAsync([
            "profile-add-excluded-error", workspace.ProjectRootPath, "DITA_TEST", error.Name]);
    ...
    {
        new[] { "profile-add-excluded-error" },
        new[] { "profile-add-excluded-error", "." },
        new[] { "profile-add-excluded-error", ".", "DITA_TEST" },
        new[] { "profile-add-excluded-error", ".", "DITA_TEST", "ERROR", "extra" }
    };
    ...
    {
        Assert.Equal(1, await ProfileAddExcludedErrorCommand.ExecuteAsync(args));
    }
