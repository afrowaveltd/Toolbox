...
public sealed class ProfileAddTagCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndPersistsCanonicalTag()
    {
        ...
        Assert.Equal("profile-add-tag", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(canonicalTag, data.GetProperty("addedTag").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.Contains(canonicalTag, saved.IncludeTags, StringComparer.OrdinalIgnoreCase);
    }
    ...
    public async Task ExecuteAsync_WithJsonBeforeTag_AddsTag()
    {
        ...
        Assert.Equal(
            ...
            document.RootElement.GetProperty("data").GetProperty("addedTag").GetString());
    }
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await ProfileAddTagCommand.ExecuteAsync(args);
