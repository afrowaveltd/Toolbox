...
public sealed class ProfileRemoveCategoryCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndRemovesCanonicalCategory()
    {
        ...
        Assert.Equal("profile-remove-category", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(category.Name, data.GetProperty("removedCategory").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.DoesNotContain(category.Name, saved.IncludeCategories, StringComparer.OrdinalIgnoreCase);
    }
    ...
    public async Task ExecuteAsync_WithJsonBeforeCategory_RemovesCategory()
    {
        ...
        Assert.Equal(
            category.Name,
            document.RootElement.GetProperty("data").GetProperty("removedCategory").GetString());
    }
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_REMOVE_CATEGORY_TEST",
                "JSON Remove Category Test");
