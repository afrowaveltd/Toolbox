...
public sealed class ProfileAddExcludedErrorCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_AddsCanonicalErrorId()
    {
        ...
        Assert.Equal("profile-add-excluded-error", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(error.Id, data.GetProperty("addedExcludedError").GetString());
        Assert.Contains(error.Id, data.GetProperty("profile").GetProperty("excludeErrors")
    ...
    public async Task ExecuteAsync_WithJsonBeforeLookup_AddsError()
    {
        ...
        Assert.Equal(error.Id,
            document.RootElement.GetProperty("data").GetProperty("addedExcludedError").GetString());
    }

    [Fact]
    public async Task ExecuteAsync_WithDuplicateError_WritesFailureWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(JsonValueKind.Null, data.GetProperty("addedExcludedError").ValueKind);
    ...
    {
        int exitCode = await ProfileAddExcludedErrorCommand.ExecuteAsync([
            "profile-add-excluded-error", ".", "WEB", "AFW_NET_0001", "--json", "--json"]);
    ...
    private static async Task<(int ExitCode, string Output)> CaptureAsync(string[] args)
    {
        ...
        try
        {
            ...
            return (await ProfileAddExcludedErrorCommand.ExecuteAsync(args), writer.ToString());
        }
        finally
        {
    ...
    private static async Task<ErrorProfileDefinition> AddProfileAsync(string path)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                path, "JSON_EXCLUDED_ERROR_TEST", "JSON Excluded Error Test");
