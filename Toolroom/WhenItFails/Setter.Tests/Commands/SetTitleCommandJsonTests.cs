...
public sealed class SetTitleCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_UpdatesTitleAndWritesStableEnvelope()
    {
        ...
        Assert.Equal("set-title", root.GetProperty("command").GetString());
        ...
        Assert.Equal(
            "Updated JSON title",
            data.GetProperty("error").GetProperty("title").GetString());
        ...
        Assert.Equal("Updated JSON title", saved.Title);
    }

    [Fact]
    public async Task ExecuteAsync_WithJsonBeforeTitle_UpdatesTitle()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorDefinition target = (await LoadErrorsAsync(workspace.WhenItFailsJsonsPath)).Errors.First();
    ...
    public async Task ExecuteAsync_WithDuplicateJsonSwitch_ReturnsInputErrorWithoutBackup()
    {
        ...
        int exitCode = await SetTitleCommand.ExecuteAsync(
        [
            "set-title",
            workspace.ProjectRootPath,
            target.Id,
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await SetTitleCommand.ExecuteAsync(args);
