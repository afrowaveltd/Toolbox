...
public sealed class SuggestDocumentationKeyCaseInsensitiveCollisionJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithUppercaseExistingKey_ReturnsCanonicalJsonSuffixWithoutWriting()
    {
        ...
        Assert.Equal(existing.PrimaryCategory, data.GetProperty("category").GetString());
        Assert.Equal(existing.Title, data.GetProperty("title").GetString());
        Assert.Equal(
            $"{existing.DocumentationKey}-2",
