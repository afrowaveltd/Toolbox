...
public sealed class ProfileRemoveMetadataCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndRemovesNormalizedMetadata()
    {
        ...
        Assert.Equal("profile-remove-metadata", root.GetProperty("command").GetString());
        ...
        Assert.Equal(canonicalKey, data.GetProperty("removedMetadataKey").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.False(saved.Metadata.TryGet(canonicalKey, out _));
    }
    ...
    public async Task ExecuteAsync_WithJsonBeforeMetadataKey_RemovesMetadata()
    {
        ...
        using JsonDocument document = JsonDocument.Parse(output);
        Assert.Equal(
            TextKeyNormalizer.NormalizeKey("release.channel"),
            document.RootElement.GetProperty("data").GetProperty("removedMetadataKey").GetString());
    }
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await ProfileRemoveMetadataCommand.ExecuteAsync(args);
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_REMOVE_METADATA_TEST",
                "JSON Remove Metadata Test");
