...
public sealed class SuggestDocumentationKeyCommandNormalizedAliasJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithLowercasePaddedAlias_ReturnsCanonicalCategory()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorCategoryDefinition category = await LoadCategoryWithAliasAsync(
            workspace.WhenItFailsJsonsPath);
        string normalizedInput = $"  {category.Aliases[0].ToLowerInvariant()}  ";
        ...
        Assert.Equal("Normalized alias", data.GetProperty("title").GetString());
        Assert.Contains(
            $"/{category.Name.ToLowerInvariant().Replace('_', '-')}/",
