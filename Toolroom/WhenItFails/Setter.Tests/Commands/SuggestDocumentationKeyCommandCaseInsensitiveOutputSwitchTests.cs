...
public sealed class SuggestDocumentationKeyCommandCaseInsensitiveOutputSwitchTests
{
    [Fact]
    public async Task ExecuteAsync_WithUppercasePlainSwitch_ReturnsPlainSuggestion()
    {
        ...
        [
            ...
            category.Name,
            "Uppercase output switch",
            "--PLAIN"
        ]);

        Assert.Equal(0, exitCode);
        Assert.Equal(
            $"when-it-fails/errors/{category.Name.ToLowerInvariant().Replace('_', '-')}/uppercase-output-switch",
            output.Trim());
    }
