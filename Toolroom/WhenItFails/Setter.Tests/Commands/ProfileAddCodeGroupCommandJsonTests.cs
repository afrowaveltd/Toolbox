...
public sealed class ProfileAddCodeGroupCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndPersistsCanonicalCodeGroup()
    {
        ...
        Assert.Equal("profile-add-code-group", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(codeGroup.Name, data.GetProperty("addedCodeGroup").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.Contains(codeGroup.Name, saved.IncludeCodeGroups, StringComparer.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task ExecuteAsync_WithJsonBeforeCodeGroup_AddsCodeGroup()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(
            codeGroup.Name,
            document.RootElement.GetProperty("data").GetProperty("addedCodeGroup").GetString());
    }

    [Fact]
    public async Task ExecuteAsync_WithDuplicateCodeGroupAndJson_WritesStructuredFailureWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(JsonValueKind.Null, data.GetProperty("addedCodeGroup").ValueKind);
    ...
    public async Task ExecuteAsync_WithDuplicateJsonSwitch_ReturnsInputErrorWithoutBackup()
    {
        ...
        int exitCode = await ProfileAddCodeGroupCommand.ExecuteAsync(
        [
            "profile-add-code-group",
            workspace.ProjectRootPath,
            profile.Name,
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await ProfileAddCodeGroupCommand.ExecuteAsync(args);
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_CODE_GROUP_TEST",
                "JSON Code Group Test");
