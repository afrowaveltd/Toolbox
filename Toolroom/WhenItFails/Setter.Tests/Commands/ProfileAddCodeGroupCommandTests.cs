...
public sealed class ProfileAddCodeGroupCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidPrefix_AddsCanonicalCodeGroupAndReturnsSuccess()
    {
        ...
        Response<ErrorProfileDefinition> addProfileResponse =
            await editor.AddProfileAsync(workspace.ProjectRootPath, "DITA_TEST", "DiTa Test");
        ...
        Assert.Contains(codeGroup.Name, savedProfile.IncludeCodeGroups);
    }

    [Fact]
    public async Task ExecuteAsync_WithCodeGroupAlreadyIncluded_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ...
        int exitCode = await ProfileAddCodeGroupCommand.ExecuteAsync(
            ["profile-add-code-group", workspace.ProjectRootPath, "DITA_TEST", codeGroup.Name]);
    ...
    public async Task ExecuteAsync_WithUnknownCodeGroup_ReturnsEditFailure()
    {
        ...
        int exitCode = await ProfileAddCodeGroupCommand.ExecuteAsync(
            ["profile-add-code-group", workspace.ProjectRootPath, "DITA_TEST", "DOES_NOT_EXIST"]);
    ...
    public static TheoryData<string[]> MissingArgumentCases =>
        new()
        {
            new[] { "profile-add-code-group" },
            new[] { "profile-add-code-group", "." },
            new[] { "profile-add-code-group", ".", "DITA_TEST" }
        };
    ...
    {
        Assert.Equal(1, await ProfileAddCodeGroupCommand.ExecuteAsync(args));
    }
    ...
    {
        int exitCode = await ProfileAddCodeGroupCommand.ExecuteAsync(
            ["profile-add-code-group", ".", "DITA_TEST", "NET", "Unexpected"]);
