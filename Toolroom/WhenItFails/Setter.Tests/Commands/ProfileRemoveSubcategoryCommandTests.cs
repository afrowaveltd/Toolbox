...
public sealed class ProfileRemoveSubcategoryCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidArguments_RemovesSubcategoryAndReturnsSuccess()
    {
    ...
    [Fact]
    public async Task ExecuteAsync_WithSubcategoryNotIncluded_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace temporaryWorkspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
    ...
    public static TheoryData<string[]> MissingArgumentCases =>
        new()
        {
            new[] { "profile-remove-subcategory" },
            new[] { "profile-remove-subcategory", "." },
            new[] { "profile-remove-subcategory", ".", "DITA_TEST" }
        };
