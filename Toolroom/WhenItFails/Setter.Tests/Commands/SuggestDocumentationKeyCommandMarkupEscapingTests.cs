...
public sealed class SuggestDocumentationKeyCommandMarkupEscapingTests
{
    [Fact]
    public async Task ExecuteAsync_WithMarkupLikeTitle_ShowsTitleLiterally()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorCategoryDefinition category = await LoadFirstCategoryAsync(
            workspace.WhenItFailsJsonsPath);
        const string title = "[red]Connection[/] timeout";
