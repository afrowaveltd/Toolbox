...
public sealed class ProfileRemoveTagCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidArguments_RemovesTagAndReturnsSuccess()
    {
        ...
        int exitCode = await ProfileRemoveTagCommand.ExecuteAsync([
            "profile-remove-tag", workspace.ProjectRootPath, " dita test ", tag.ToLowerInvariant()]);

        Assert.Equal(0, exitCode);
        ErrorProfileDefinition profile = await LoadProfileAsync(workspace.WhenItFailsJsonsPath);
        Assert.DoesNotContain(TextKeyNormalizer.NormalizeKey(tag), profile.IncludeTags);
    }

    [Fact]
    public async Task ExecuteAsync_WithTagNotIncluded_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace workspace = await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        WhenItFailsProfileWorkspaceEditor editor = new();
        Assert.True((await editor.AddProfileAsync(workspace.ProjectRootPath, "DITA_TEST", "DiTa Test")).IsSuccess);
        ...
        Assert.Equal(2, await ProfileRemoveTagCommand.ExecuteAsync([
            "profile-remove-tag", workspace.ProjectRootPath, "DITA_TEST", tag]));
    }
    ...
    public async Task ExecuteAsync_WithUnknownTag_ReturnsEditFailure()
    {
        ...
        Assert.Equal(2, await ProfileRemoveTagCommand.ExecuteAsync([
            "profile-remove-tag", workspace.ProjectRootPath, "DITA_TEST", "DOES_NOT_EXIST"]));
    }

    public static TheoryData<string[]> MissingArgumentCases => new()
    {
        new[] { "profile-remove-tag" },
        new[] { "profile-remove-tag", "." },
        new[] { "profile-remove-tag", ".", "DITA_TEST" }
    };
    ...
    public async Task ExecuteAsync_WithMissingRequiredArgument_ReturnsCommandInputError(string[] args) =>
        Assert.Equal(1, await ProfileRemoveTagCommand.ExecuteAsync(args));

    [Fact]
    public async Task ExecuteAsync_WithTooManyArguments_ReturnsCommandInputError() =>
        Assert.Equal(1, await ProfileRemoveTagCommand.ExecuteAsync([
            "profile-remove-tag", ".", "DITA_TEST", "NETWORK", "Unexpected"]));
