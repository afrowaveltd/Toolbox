...
public sealed class ProfileAddExcludedTagCommandTests
{
    [Fact]
    public async Task ExecuteAsync_WithValidArguments_AddsExcludedTagAndReturnsSuccess()
    {
        ...
        int exitCode = await ProfileAddExcludedTagCommand.ExecuteAsync(
            ["profile-add-excluded-tag", workspace.ProjectRootPath, " dita test ", tag.ToLowerInvariant()]);

        Assert.Equal(0, exitCode);
        ErrorProfileDefinition profile = await LoadProfileAsync(
            workspace.WhenItFailsJsonsPath,
            "DITA_TEST");
        Assert.Contains(TextKeyNormalizer.NormalizeKey(tag), profile.ExcludeTags);
    }

    [Fact]
    public async Task ExecuteAsync_WithTagAlreadyExcluded_ReturnsEditFailure()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        WhenItFailsProfileWorkspaceEditor editor = new();
        ...
        int exitCode = await ProfileAddExcludedTagCommand.ExecuteAsync(
            ["profile-add-excluded-tag", workspace.ProjectRootPath, "DITA_TEST", tag]);
    ...
    public async Task ExecuteAsync_WithUnknownTag_ReturnsEditFailure()
    {
        ...
        int exitCode = await ProfileAddExcludedTagCommand.ExecuteAsync(
            ["profile-add-excluded-tag", workspace.ProjectRootPath, "DITA_TEST", "DOES_NOT_EXIST"]);
    ...
    public static TheoryData<string[]> MissingArgumentCases =>
        new()
        {
            new[] { "profile-add-excluded-tag" },
            new[] { "profile-add-excluded-tag", "." },
            new[] { "profile-add-excluded-tag", ".", "DITA_TEST" }
        };
    ...
    {
        Assert.Equal(1, await ProfileAddExcludedTagCommand.ExecuteAsync(args));
    }
    ...
    public async Task ExecuteAsync_WithTooManyArguments_ReturnsCommandInputError()
    {
        Assert.Equal(
            1,
            await ProfileAddExcludedTagCommand.ExecuteAsync(
                ["profile-add-excluded-tag", ".", "DITA_TEST", "RETRYABLE", "Unexpected"]));
    }
