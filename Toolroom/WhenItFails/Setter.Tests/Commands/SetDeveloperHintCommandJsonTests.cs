...
public sealed class SetDeveloperHintCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_UpdatesHintAndWritesStableEnvelope()
    {
        ...
        Assert.Equal("set-developer-hint", root.GetProperty("command").GetString());
        ...
        Assert.Equal(
            "Check the network adapter state.",
            data.GetProperty("error").GetProperty("developerHint").GetString());
        ...
        Assert.Equal("Check the network adapter state.", saved.DeveloperHint);
    }

    [Fact]
    public async Task ExecuteAsync_WithJsonBeforeHint_UpdatesHint()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorDefinition target = (await LoadErrorsAsync(workspace.WhenItFailsJsonsPath)).Errors.First();
    ...
    public async Task ExecuteAsync_WithDuplicateJsonSwitch_ReturnsInputErrorWithoutBackup()
    {
        ...
        int exitCode = await SetDeveloperHintCommand.ExecuteAsync(
        [
            "set-developer-hint",
            workspace.ProjectRootPath,
            target.Id,
    ...
    private static async Task<(int ExitCode, string Output)> ExecuteWithCapturedOutputAsync(string[] args)
    {
        ...
        try
        {
            ...
            int exitCode = await SetDeveloperHintCommand.ExecuteAsync(args);
