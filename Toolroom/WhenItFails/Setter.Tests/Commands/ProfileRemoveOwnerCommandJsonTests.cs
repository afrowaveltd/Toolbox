...
public sealed class ProfileRemoveOwnerCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndRemovesCanonicalOwner()
    {
        ...
        Assert.Equal("profile-remove-owner", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(owner.Name, data.GetProperty("removedOwner").GetString());
        ...
        ErrorProfileDefinition saved = await LoadProfileAsync(
            ...
            profile.Name);
        Assert.DoesNotContain(owner.Name, saved.IncludeOwners, StringComparer.OrdinalIgnoreCase);
    }

    [Fact]
    public async Task ExecuteAsync_WithJsonBeforeOwner_RemovesOwner()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(
            owner.Name,
            document.RootElement.GetProperty("data").GetProperty("removedOwner").GetString());
    }

    [Fact]
    public async Task ExecuteAsync_WithOwnerNotIncludedAndJson_WritesStructuredFailureWithoutBackup()
    {
        using TemporaryWhenItFailsWorkspace workspace =
            await TemporaryWhenItFailsWorkspace.CreateInitializedAsync();
        ErrorProfileDefinition profile = await AddTestProfileAsync(workspace.ProjectRootPath);
        ...
        Assert.Equal(JsonValueKind.Null, data.GetProperty("removedOwner").ValueKind);
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_REMOVE_OWNER_TEST",
                "JSON Remove Owner Test");
