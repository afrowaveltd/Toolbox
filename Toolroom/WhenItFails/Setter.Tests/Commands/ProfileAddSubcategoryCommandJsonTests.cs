...
public sealed class ProfileAddSubcategoryCommandJsonTests
{
    [Fact]
    public async Task ExecuteAsync_WithJsonOutput_WritesStableEnvelopeAndPersistsCanonicalSubcategory()
    {
        ...
        string lookup = subcategory.ToLowerInvariant();
        ...
        Assert.Equal("profile-add-subcategory", root.GetProperty("command").GetString());
        Assert.True(data.GetProperty("updated").GetBoolean());
        Assert.Equal(canonical, data.GetProperty("addedSubcategory").GetString());
        Assert.Contains(
        ...
        Assert.Contains(canonical, saved.IncludeSubcategories, StringComparer.OrdinalIgnoreCase);
    }
    ...
    public async Task ExecuteAsync_WithJsonBeforeSubcategory_AddsSubcategory()
    {
        ...
        Assert.Equal(
            ...
            document.RootElement.GetProperty("data").GetProperty("addedSubcategory").GetString());
    }
    ...
    private static async Task<ErrorProfileDefinition> AddTestProfileAsync(string projectRootPath)
    {
        Response<ErrorProfileDefinition> response =
            await new WhenItFailsProfileWorkspaceEditor().AddProfileAsync(
                projectRootPath,
                "JSON_SUBCATEGORY_TEST",
                "JSON Subcategory Test");
