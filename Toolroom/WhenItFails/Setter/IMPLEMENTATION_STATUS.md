...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining public `JsonsOptions` filename boundaries. Avoid four copy-pasted one-off contracts; prefer one parameterized red-first contract for the remaining category, code-group, owner, and profile file-name properties if current runtime paths can still throw before provider invocation. Then continue to adjacent composition/runtime helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogContextProviderNullErrorCatalogFileNameContractTests` is user-verified green: **1 passed**. A runtime-null `JsonsOptions.ErrorCatalogFileName` no longer escapes from `ErrorCatalogContextProvider.LoadFromJsonsAsync` as `ArgumentNullException`; the method now returns stable Invalid response `WIF_JSONS_ERROR_CATALOG_FILE_NAME_NULL` with message `The error catalog file name cannot be null.` before invoking the error catalog provider. Production commit: `cbd4d26a76729e42e6c26b26479c1e671ec26dee`. Red-first test commit: `99b3663f9b0a29171128d132f3abbb0214a9e0f2`.
