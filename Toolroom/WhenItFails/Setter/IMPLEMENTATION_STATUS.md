...
Continue the runtime/public-API audit from the **872-test** green baseline beyond `ErrorCatalogRuntime`, `ErrorDescriptorResolver`, and `CatalogProviderPipeline`. Inspect the remaining resolver/provider/bootstrap composition boundaries for direct dereferences or enumerations of publicly mutable dependency-result collections, especially `Response<T>.Issues`, and add a red-first contract only where the current runtime path can actually throw.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`CatalogProviderPipelineNullIssuesCollectionContractTests` is user-verified green: **1 passed**. A failed catalog loader response with `Issues = null` no longer throws from `CatalogProviderPipeline.GetFirstIssueCode`; the pipeline returns the caller-supplied fallback load failure code/message while preserving the dependency failure status. Production commit: `54b0bda9d8445534e51820ecd61fef2c8ec113ae`. Red-first test commit: `ba5cffb4a3310210973dbd43574056c6267eda0e`.
