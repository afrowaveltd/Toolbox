...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining public `JsonsOptions` path boundaries. All five catalog file-name properties are now protected for both runtime-null and empty/whitespace values before provider invocation. Next inspect `RootDirectory` and `PackageDirectoryName` for malformed public values that can still escape as path exceptions or redirect operations unexpectedly; add a red-first contract only for a proven runtime gap, then continue to adjacent composition/runtime helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogContextProviderWhitespaceCatalogFileNameContractTests` is user-verified green: **5 passed**. Empty/whitespace values for `ErrorCatalogFileName`, `CategoryCatalogFileName`, `CodeGroupCatalogFileName`, `OwnerCatalogFileName`, and `ProfilesFileName` are now rejected during `ErrorCatalogContextProvider` preflight validation before any provider invocation, with stable Invalid `*_EMPTY` codes and catalog-specific messages. Production commit: `d48473d79365d033bbcfdbbf2ae8da1501059c56`. Red-first test commit: `ffdabb0ff0ecc4b108458a95cf497331bd0fc1e0`.
