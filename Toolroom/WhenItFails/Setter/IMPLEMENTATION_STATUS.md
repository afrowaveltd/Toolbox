...
Continue the runtime/public-API audit from the **872-test** green baseline into adjacent loading/provider boundaries. `JsonCatalogDocumentWriter` now has a stable malformed-path response, so inspect catalog loaders/providers for equivalent public file-path exceptions or malformed dependency values that can still escape as exceptions or unstable responses. Add one narrow red-first contract for the next proven gap; avoid duplicating already-protected empty/null path semantics.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonCatalogDocumentWriterInvalidFilePathContractTests` is user-verified green: **1 passed**. A target path containing an embedded null character no longer escapes `JsonCatalogDocumentWriter.SaveToFileAsync` as `ArgumentException`; the writer now returns stable Invalid response `FilePathIsInvalid` with message `JSON catalog file path is invalid.` Existing cancellation, access-denied, I/O, and JSON serialization behavior remains unchanged. Production commit: `aa51189cf627818a78723c593ec13ac8b165db77`. Red-first test commit: `068118260cc8041da33c96b452920dd0a6ad7278`.
