...
Continue the runtime/public-API audit from the **872-test** green baseline with `categoryCatalog.Categories` in `ErrorCatalogCrossValidator`. It is still dereferenced through `.Count` and index construction without a collection-null guard. Add one red-first contract if no equivalent test exists, then keep `Profiles` as a separate follow-up slice.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullCodeGroupsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null supporting `ErrorCodeGroupCatalogDocument.CodeGroups` collection now becomes `CodeGroupCatalogCodeGroupsCollectionIsNull` at `codeGroupCatalog.codeGroups`; code-group-specific iteration and index construction safely skip that malformed supporting collection instead of throwing.