...
Continue the runtime/public-API audit from the **872-test** green baseline with `codeGroupCatalog.CodeGroups` in `ErrorCatalogCrossValidator`. It is still dereferenced through `.Count` and index construction without a collection-null guard. Add one red-first contract if no equivalent test exists, then keep `Categories` and `Profiles` as separate follow-up slices.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullOwnersCollectionContractTests` is user-verified green: **1 passed**. A runtime-null supporting `ErrorOwnerCatalogDocument.Owners` collection now becomes `OwnerCatalogOwnersCollectionIsNull` at `ownerCatalog.owners`; owner-specific iteration and index construction safely skip that malformed supporting collection instead of throwing.