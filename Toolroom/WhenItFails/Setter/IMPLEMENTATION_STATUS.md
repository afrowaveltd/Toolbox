...
Continue the runtime/public-API audit from the **872-test** green baseline through adjacent loading/provider boundaries. Writer and shared document loader malformed-path semantics are now stable and aligned. Inspect concrete catalog providers for malformed mutable dependency responses or status/issue forwarding gaps, and add one narrow red-first contract for the next proven runtime/public-API gap. Avoid duplicating the shared loader path contract in every concrete loader/provider.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonCatalogDocumentLoaderInvalidFilePathContractTests` is user-verified green: **1 passed**. A source path containing an embedded null character is no longer masked by `File.Exists` as `NotFound`; `JsonCatalogDocumentLoader.LoadFromFileAsync` now returns stable Invalid response `FilePathIsInvalid` with message `JSON catalog file path is invalid.` before existence checking. Valid missing paths retain the existing `NotFound` / `FileNotFound` behavior. Production commit: `a9de959d872ea62897a93a9a30fd4829efb376a7`. Red-first test commit: `d97d8dee3b301e05541f76313ece3a0f1fa14cda`.
