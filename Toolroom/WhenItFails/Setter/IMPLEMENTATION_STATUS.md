...
Continue the runtime/public-API audit from the **872-test** green baseline by moving to a semantically distinct malformed nested-value boundary outside the category string-list family. Inspect remaining owner, code-group, or profile definition collections/dictionaries and add a red-first contract only where the public validator still turns runtime-null mutable state into an exception.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCategoryCatalogValidatorNullParentCategoriesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCategoryDefinition.ParentCategories` collection now becomes `CategoryParentCategoriesCollectionIsNull` at `categories[index].parentCategories` instead of throwing `NullReferenceException` in the shared collection helper.