...
Continue the runtime/public-API audit with `ErrorCodeGroupDefinition`.

Add focused coverage for safe scalar defaults and verify that default categories, default tags, default mappings, and metadata are independently allocated between code-group instances. Keep the test namespace under `DefinitionContracts` to avoid shadowing the production `Definitions` namespace.
...
`ErrorCategoryCatalogDocumentContractTests` passed all **2 focused tests**. Category catalog documents now have direct coverage for schema/language defaults and independently allocated mutable containers.