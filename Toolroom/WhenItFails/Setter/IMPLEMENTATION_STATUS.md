...
Continue the runtime/public-API audit from the **872-test** green baseline by inspecting the remaining standalone catalog validators for malformed collection-item boundaries. Next inspect the main `ErrorCatalogValidator`; add a red-first contract only if a runtime-null error definition can still escape as an exception.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCategoryCatalogValidatorNullCategoryDefinitionContractTests` is user-verified green: **1 passed**. A runtime-null item in `ErrorCategoryCatalogDocument.Categories` now becomes `CategoryDefinitionIsNull` at `categories[index]` instead of throwing `NullReferenceException`, including safe handling in normalized-name processing.