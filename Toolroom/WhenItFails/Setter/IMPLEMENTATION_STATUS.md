...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining public `JsonsOptions` path boundaries. Runtime-null `RootDirectory` and `PackageDirectoryName` are now protected before derived path evaluation. Next inspect empty/whitespace workspace path values as one parameterized contract; if malformed values still reach providers or filesystem paths, add a narrow red-first contract and stable Invalid responses. Then continue to adjacent composition/runtime helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogContextProviderNullWorkspacePathOptionsContractTests` is user-verified green: **2 passed**. Runtime-null `JsonsOptions.RootDirectory` and `JsonsOptions.PackageDirectoryName` no longer escape from `ErrorCatalogContextProvider.LoadFromJsonsAsync` through `Path.Combine`; preflight validation now returns stable Invalid responses `WIF_JSONS_ROOT_DIRECTORY_NULL` / `The JSON root directory cannot be null.` and `WIF_JSONS_PACKAGE_DIRECTORY_NAME_NULL` / `The package directory name cannot be null.` before any derived path getter or catalog provider invocation. Production commit: `ff4f804e81382f41095366407d615e89b08a6340`. Red-first test commit: `73318770d803c437eb0bb3be2aecdd28143bac15`.
