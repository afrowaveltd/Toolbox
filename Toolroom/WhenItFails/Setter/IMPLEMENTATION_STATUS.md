...
Continue the runtime/public-API audit from the **872-test** green baseline with the next isolated malformed dependency-response boundary in `ErrorCatalogRuntime`. `ForwardContextFailure` still dereferences `sourceResponse.Issues.Count` directly when a context-store response is unsuccessful, even though `Response<T>.Issues` is publicly settable and can be runtime-null. Add one red-first public contract through a facade method such as `FromId`, using a failing context-store response with `Issues = null!`, and require the stable fallback code `ErrorCatalogContextUnavailable` instead of an exception.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogRuntimeNullFallbackIssuesCollectionContractTests` is user-verified green: **1 passed**. A malformed built-in fallback dependency response with `Issues = null` no longer throws from `CreateBuiltInFallbackFailureResponse`; the runtime returns the stable fallback failure response and derives `WhenItFails.FallbackFailure.Code` from the missing payload/load state. Production commit: `7e5822de5380a1835ba22460e34e43730200531a`. Red-first test commit: `3cf702dbc53f87a6e3ce2f55b07c2ef6ba52048f`.
