...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining public `JsonsOptions` filename boundaries. Null handling is now covered for all five catalog file names before provider invocation. Next inspect empty/whitespace file names as one parameterized contract rather than five one-off tests, then continue to adjacent composition/runtime helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogContextProviderNullSupportingCatalogFileNameContractTests` is user-verified green: **4 passed**. Runtime-null `CategoryCatalogFileName`, `CodeGroupCatalogFileName`, `OwnerCatalogFileName`, and `ProfilesFileName` values are now rejected during preflight validation before the error catalog provider or any downstream catalog provider is invoked. Stable Invalid codes are `WIF_JSONS_CATEGORY_CATALOG_FILE_NAME_NULL`, `WIF_JSONS_CODE_GROUP_CATALOG_FILE_NAME_NULL`, `WIF_JSONS_OWNER_CATALOG_FILE_NAME_NULL`, and `WIF_JSONS_PROFILE_CATALOG_FILE_NAME_NULL`. Production commit: `698e12b5b2be1ea0b14ebf6775f3c52bf4e993f1`. Red-first test commit: `77e0027a65e6154ba5b1093ae6598fe23529b854`.
