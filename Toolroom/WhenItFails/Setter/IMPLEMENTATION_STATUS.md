...
Continue the runtime/public-API audit from the **872-test** green baseline beyond the now-protected bootstrap/template path boundary. `JsonsBootstrapper.IsPathInsideDirectory` already covers relative traversal and absolute paths that resolve outside the package directory, so avoid duplicate path-shape tests unless platform-specific behavior exposes a distinct contract gap. Continue with adjacent composition/runtime helpers and add a red-first contract only where a current malformed dependency value can produce an unstable response or exception.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonsBootstrapperEscapingTemplateTargetFileNameContractTests` is user-verified green: **1 passed**. The focused contract confirms the existing production `IsPathInsideDirectory` guard rejects `../escaped.json` before any write outside the package directory and returns stable Invalid response `WIF_JSONS_TEMPLATE_TARGET_FILE_NAME_OUTSIDE_PACKAGE` with message `The JSON template provider returned a target file name outside the package directory.` The initial test commit was `5305b5260703440d03b4f91b704250e124d03d96`; the assertion was aligned with the pre-existing production message in test-only commit `395a0643c938a1703d7b813144b12909fda3206c`.
