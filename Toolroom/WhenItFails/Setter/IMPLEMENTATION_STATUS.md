...
Continue the runtime/public-API audit from the **872-test** green baseline by inspecting `ErrorCodeGroupDefinition` and its standalone validator for a semantically real nested mutable collection dereference. Prefer a collection actually passed into shared validation or used by code-group-specific logic, and add only one red-first contract after confirming no equivalent test already exists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorOwnerCatalogValidatorNullAliasesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorOwnerDefinition.Aliases` collection now becomes `OwnerAliasesCollectionIsNull` at `owners[index].aliases` instead of throwing in shared string collection validation or the owner-specific alias loop.