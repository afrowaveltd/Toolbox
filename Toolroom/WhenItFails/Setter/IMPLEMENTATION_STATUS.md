...
Continue the runtime/public-API audit from the **872-test** green baseline with the next isolated malformed dependency-response boundary in `ErrorCatalogRuntime`. `RecordStatus` still dereferences `recoveryReason.Issues.Count` directly while recording a successful flexible recovery, even though `Response<T>.Issues` is publicly settable and can be runtime-null. Add one red-first public contract using a failing initializer response with `Issues = null!`, a valid previous context, and `HideRecoverableFailures = true`, so the first proven failure is the status-recording path rather than warning-detail formatting.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogRuntimeNullContextIssuesCollectionContractTests` is user-verified green: **1 passed**. A malformed context-store failure response with `Issues = null` no longer throws from `ForwardContextFailure`; facade calls such as `FromId` now return a stable failure using fallback code `ErrorCatalogContextUnavailable`. Production commit: `2da5e3f275dbf2543931982f33ac01cc60581797`. Red-first test commit: `8a6637ab8bafc815cf55d1c48d801dcd6a6af31e`.
