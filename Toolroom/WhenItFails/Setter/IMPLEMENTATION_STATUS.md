...
Continue the runtime/public-API audit from the **872-test** green baseline with a semantically distinct nested mutable collection rather than extending the now-complete document-level family. Inspect `ErrorOwnerDefinition.Aliases`: it is passed to shared string collection validation and then enumerated again by owner-specific alias conflict logic, so a runtime-null value may currently throw in more than one public validation path.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogValidatorNullErrorsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCatalogDocument.Errors` collection now becomes `CatalogErrorsCollectionIsNull` at `errors` instead of throwing `NullReferenceException` on the first `.Count` dereference. The document-level mutable collection family is now complete across all five standalone catalog validators.