...
Continue the runtime/public-API audit with focused coverage for initializer `Success` responses whose non-null initialization payload contains an unusable required nested value such as a null `Bootstrap` or null `Context`.

Verify `InitializeAsync` returns stable non-null `Invalid` responses rather than throwing or accepting an internally invalid successful payload. Preserve the complete **780-test** core baseline until the next full regression run.

## Last completed change

`ErrorCatalogRuntimeNullInitializationPayloadContractTests` passed its **1 focused test** after runtime hardening. An initializer `Success` response with `Data == null` is now rejected as `WIF_INITIALIZATION_PAYLOAD_NULL` instead of being reported as a successful initialization.