...
Continue the runtime/public-API audit across the remaining individual catalog providers. Inspect the category, code-group, owner, and profile provider pipelines for runtime-null loader responses, normalizer results, validator results, or malformed successful payloads, while avoiding redundant contracts where shared behavior is already structurally identical and well covered.

Prefer one narrow provider-family contract with a clear public response shape. Preserve the complete **780-test** core baseline until the next full regression run.

## Last completed change

`ErrorCatalogProviderNullDependencyResultContractTests` passed all **4 focused tests** after provider hardening. Runtime-null results from the loader, normalizer, validator, and factory now become layer-specific stable `Invalid` responses instead of `NullReferenceException` or malformed successful payloads.