...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining bootstrap/template boundaries. Inspect `JsonsBootstrapper` template path/name handling for malformed dependency-returned values that can currently be interpreted as valid filesystem operations or leak infrastructure-shaped failures, and add a red-first contract only for a proven boundary. Then continue to adjacent composition helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonsBootstrapperNullTemplateContentContractTests` is user-verified green: **1 passed**. A template returned by `IJsonsTemplateProvider` with runtime-null `Content` no longer passes through as a successful bootstrap file; `JsonsBootstrapper.EnsureWorkspaceAsync` now returns stable Invalid response `WIF_JSONS_TEMPLATE_CONTENT_NULL` with message `The JSON template provider returned a template with null content.` before any write. Production commit: `3e062be8b3b0637cb8fc1c0efa44ca9bb85b68ef`. Red-first test commit: `2f1242fc64ae580464aef6f3985554562e4b9ee9`.
