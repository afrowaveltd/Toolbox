...
Continue the runtime/public-API audit from the **872-test** green baseline with `ErrorProfileDefinition.ExcludeTags`. It is the remaining profile string collection still passed directly into shared string collection validation, so add one red-first contract if no equivalent test exists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorProfileCatalogValidatorNullIncludeTagsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.IncludeTags` collection now becomes `ProfileIncludeTagsCollectionIsNull` at `profiles[index].includeTags` instead of throwing inside shared string collection validation.