...
Continue the runtime/public-API audit from the **872-test** green baseline with the complementary `ErrorProfileDefinition.ExcludeErrors` boundary. It participates both in shared collection validation and in the indexed include/exclude conflict loop, so a runtime-null value should become a stable validation issue rather than an exception.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorProfileCatalogValidatorNullIncludeErrorsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.IncludeErrors` collection now becomes `ProfileIncludeErrorsCollectionIsNull` at `profiles[index].includeErrors` instead of throwing in shared collection validation or include/exclude conflict processing.