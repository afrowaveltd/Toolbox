...
Continue the runtime/public-API audit from the **872-test** green baseline with another document-level mutable collection boundary. Inspect `ErrorCodeGroupCatalogDocument.CodeGroups`, `ErrorCategoryCatalogDocument.Categories`, or `ErrorProfileCatalogDocument.Profiles` for the same direct `.Count`/iteration dereference pattern, and add one red-first contract at a time.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorOwnerCatalogValidatorNullOwnersCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorOwnerCatalogDocument.Owners` collection now becomes `OwnerCatalogOwnersCollectionIsNull` at `owners` instead of throwing `NullReferenceException` on the first `.Count` dereference.