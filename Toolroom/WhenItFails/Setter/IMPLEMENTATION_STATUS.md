...
Continue the runtime/public-API audit from the **872-test** green baseline with `profileCatalog.Profiles` in `ErrorCatalogCrossValidator`. It is the remaining supporting document collection still dereferenced through `.Count` and profile iteration without a collection-null guard. Add one red-first contract if no equivalent test exists, then reassess the cross-validator for nested mutable collection dereferences rather than extending the series mechanically.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullCategoriesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null supporting `ErrorCategoryCatalogDocument.Categories` collection now becomes `CategoryCatalogCategoriesCollectionIsNull` at `categoryCatalog.categories`; category-specific iteration and index construction safely skip that malformed supporting collection instead of throwing.