...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining core provider/bootstrap composition boundaries. Inspect the project `ErrorCatalogContextProvider` and adjacent composition helpers for direct dereferences or enumerations of publicly mutable dependency-returned values and collections. Providers already routed exclusively through `CatalogProviderPipeline` do not need duplicate null-`Issues` contracts unless they add their own dependency-result handling.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`BuiltInErrorCatalogContextProviderNullTemplatesCollectionContractTests` is user-verified green: **1 passed**. When `IJsonsTemplateProvider.GetTemplateFiles` returns a runtime-null collection, `BuiltInErrorCatalogContextProvider.LoadAsync` now returns stable Invalid response `WIF_BUILT_IN_TEMPLATES_NULL` with message `The bundled WhenItFails catalog template provider returned a null collection.` before dereferencing the collection or invoking the downstream context provider. Production commit: `78816800c6c6f718014422e52ca1611018719b22`. Red-first test commit: `7b4a4be05b1e3e49b4b81077688681c0cbbd700e`.
