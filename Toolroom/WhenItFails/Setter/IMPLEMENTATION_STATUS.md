...
Continue the runtime/public-API audit from the **872-test** green baseline with a different ownership boundary: document-level mutable collections. `ErrorOwnerCatalogDocument.Owners` is publicly settable and the standalone validator currently dereferences `.Count` directly, so inspect a runtime-null collection contract before returning to nested definition lists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorProfileCatalogValidatorNullExcludeErrorsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.ExcludeErrors` collection now becomes `ProfileExcludeErrorsCollectionIsNull` at `profiles[index].excludeErrors` instead of throwing in shared collection validation or the indexed include/exclude conflict loop.