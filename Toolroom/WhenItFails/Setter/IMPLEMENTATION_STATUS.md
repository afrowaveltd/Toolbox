...
Continue the runtime/public-API audit from the **872-test** green baseline by re-scanning current `ErrorCatalogRuntime` for any remaining direct dereferences or enumerations of publicly mutable dependency-result collections. If no direct null-`Issues` boundary remains in this facade, move to the next runtime/public service boundary rather than mechanically adding redundant tests.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogRuntimeNullInitializationFailureIssuesCollectionContractTests` is user-verified green: **1 passed**. When flexible initialization fails, no previous context exists, the initializer failure has `Issues = null`, and the built-in fallback also fails, `AddInitializationFailureMetadata` no longer throws. `WhenItFails.ProjectFailure.Code` now falls back to `WIF_INITIALIZATION_FAILED` while the fallback failure remains represented normally. Production commit: `9e5407cf19280e86e30d4fc0c6a2a6effd18bff7`. Red-first test commit: `557f932aa9d8522b92d33c5eeb38017eb0449c95`.
