...
Continue the runtime/public-API audit from the **872-test** green baseline by inspecting the remaining mutable collections passed directly into standalone validators. Prioritize `ErrorProfileDefinition.IncludeOwners`, `IncludeCodeGroups`, `IncludeCategories`, `IncludeSubcategories`, `IncludeTags`, or `ExcludeTags` if they still flow unguarded into shared string collection validation; add only one red-first contract at a time.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCodeGroupCatalogValidatorNullDefaultTagsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCodeGroupDefinition.DefaultTags` collection now becomes `CodeGroupDefaultTagsCollectionIsNull` at `codeGroups[index].defaultTags` instead of throwing inside shared string collection validation. Together with `DefaultCategories`, the code-group nested string collection pair is now protected.