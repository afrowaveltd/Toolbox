...
Continue the runtime/public-API audit from the **872-test** green baseline with the next remaining proven null-`Issues` dereference in `ErrorCatalogRuntime`. `AddInitializationFailureMetadata` still reads `failureResponse.Issues.Count` directly while composing the project-failure metadata for a failed built-in fallback. Add one red-first public contract using flexible initialization with no previous context, an initializer failure whose `Issues = null!`, and a failing built-in response with ordinary diagnostics. Require `WhenItFails.ProjectFailure.Code` to fall back to `WIF_INITIALIZATION_FAILED` rather than throwing.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogRuntimeNullRecoveryDetailsIssuesCollectionContractTests` is user-verified green: **1 passed**. A visible previous-context recovery whose initializer failure response has `Issues = null` no longer throws from `CreateRecoveryDetails`; the warning response uses stable generic details when neither issues nor an initializer message are available. Production commit: `be281ec75216863e2f3f7100639ecc31b5ba056c`. Red-first test commit: `77da9c8d8545470b21869e4328af4b5154434c53`.
