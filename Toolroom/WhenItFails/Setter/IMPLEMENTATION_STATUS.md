...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining core provider/bootstrap composition boundaries. Inspect `BuiltInErrorCatalogContextProvider` and the project `ErrorCatalogContextProvider` for direct dereferences or enumerations of publicly mutable dependency-result collections, especially `Response<T>.Issues`, and add a red-first contract only where the current runtime path can actually throw. Providers already routed exclusively through `CatalogProviderPipeline` do not need duplicate null-`Issues` contracts unless they add their own dependency-result handling.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogInitializerNullIssuesCollectionContractTests` is user-verified green: **1 passed**. A failed bootstrapper response with `Issues = null` no longer throws from `ErrorCatalogInitializer.CreateFailedResponse`; `InitializeAsync` returns the stable fallback code `ErrorCatalogBootstrapFailed` and fallback message while preserving the dependency failure status. Because both bootstrapper and context-provider failure paths share the helper, the same null-safe behavior now applies to both. Production commit: `ddb0ecf2b0847be486bcaf709309d2e361a4a25a`. Red-first test commit: `4336af9f515d86ffdd8b46e8fa74b11e3d41d2b2`.
