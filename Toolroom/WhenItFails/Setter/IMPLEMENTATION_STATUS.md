...
Continue the runtime/public-API audit one layer lower in the individual catalog providers. Inspect loader, normalizer, validator, and factory dependency boundaries for runtime-null results or malformed success values that can produce false success or runtime exceptions.
...
`ErrorCatalogContextProviderNullDependencyResponseContractTests` passed all **5 focused tests** after context-provider hardening. Runtime-null responses from the error, category, code-group, owner, and profile catalog providers now become provider-specific stable `Invalid` responses instead of `NullReferenceException`.