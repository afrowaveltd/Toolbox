...
Continue the runtime/public-API audit from the **872-test** green baseline with the remaining proven null-`Issues` dereferences in `ErrorCatalogRuntime`. The warning-detail helper `CreateRecoveryDetails` still directly reads `initializationResponse.Issues.Count` and enumerates the collection. Add one red-first public contract using flexible initialization with a valid previous context, `HideRecoverableFailures = false`, and a failing initializer response whose `Issues = null!`; require recovery to return its warning response with stable generic details rather than throwing.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogRuntimeNullRecoveryStatusIssuesCollectionContractTests` is user-verified green: **1 passed**. A flexible previous-context recovery whose initializer failure response has `Issues = null` now completes without throwing. The recovery status remains internally consistent through stable fallback recovery code/message values, and recovery metadata uses the same stable fallback semantics. Red-first test commit: `b187eee2c92701373d71b9d1f781f75311d5a661`. Production commits while following the full public recovery path: `5c22b7cace8e896ea74329c50d6902cc204d85fa`, `6187c35f402832a1d704550156976b88778d9cf7`, and `6344f2760232b8c86c478d61039ffd593b5ff969`.
