...
Continue the runtime/public-API audit from the **872-test** green baseline with the supporting document collections consumed by `ErrorCatalogCrossValidator`. `ownerCatalog.Owners` is currently dereferenced through `.Count` and enumeration without a collection-null guard, so add one red-first contract for that boundary before moving to `CodeGroups`, `Categories`, or `Profiles`.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullErrorsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null main `ErrorCatalogDocument.Errors` collection now becomes `CatalogErrorsCollectionIsNull` at `errors` instead of throwing at the first `.Count` dereference; the cross-validator returns that invalid result because the main error catalog cannot be cross-validated without its primary collection.