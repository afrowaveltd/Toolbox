...
Continue the runtime/public-API audit from the **872-test** green baseline with the next proven nested mutable collection boundary in `ErrorCatalogCrossValidator`. `ValidateProfileIncludeCategories` dereferences `ErrorProfileDefinition.IncludeCategories` through `.Count` when the category index is non-empty, while the standalone profile validator already exposes the stable `ProfileIncludeCategoriesCollectionIsNull` contract. Add one red-first cross-validator contract with a non-empty supporting category catalog if no equivalent test exists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullProfileIncludeCodeGroupsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.IncludeCodeGroups` collection now becomes `ProfileIncludeCodeGroupsCollectionIsNull` at `profiles[index].includeCodeGroups`; the null guard runs before the empty-index shortcut, and include-code-group validation is skipped safely while the remaining independent profile cross-validation continues. Production commit: `988b025c3bde5703b0c6ac3ddd5635fee7b5da8c`. Red-first test commit: `4d0d79594d05f279c91bd4debed87157abcaf10c`.
