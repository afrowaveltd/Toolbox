...
Continue the runtime/public-API audit from the **872-test** green baseline with the remaining standalone document-level mutable collection boundary. Inspect `ErrorProfileCatalogDocument.Profiles`; if it still dereferences `.Count` directly, add one red-first contract before deciding whether the document-level collection family is complete.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCategoryCatalogValidatorNullCategoriesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCategoryCatalogDocument.Categories` collection now becomes `CategoryCatalogCategoriesCollectionIsNull` at `categories` instead of throwing `NullReferenceException` during count, normalization, or iteration.