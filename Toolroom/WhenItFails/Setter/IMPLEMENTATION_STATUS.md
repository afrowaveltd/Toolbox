...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining core provider/bootstrap composition boundaries. Inspect for direct dereferences or enumerations of publicly mutable dependency-result collections, especially `Response<T>.Issues`, and add a red-first contract only where the current runtime path can actually throw. Providers already routed exclusively through `CatalogProviderPipeline` do not need duplicate null-`Issues` contracts unless they add their own dependency-result handling.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogProviderNullIssuesCollectionContractTests` is user-verified green: **1 passed**. A failed `IErrorCatalogLoader` response with `Issues = null` no longer throws from `ErrorCatalogProvider.GetFirstIssueCode`; `LoadFromFileAsync` returns the stable fallback code `CatalogLoadFailed` and fallback message while preserving the dependency failure status. Production commit: `d113c5781a1c46a40b6a96bf0c5fd1e4c4b23da3`. Red-first test commit: `db81d8d2cdf70e64fb020df4ca85648653e2b3ce`.
