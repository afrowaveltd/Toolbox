...
Continue the runtime/public-API audit from the **872-test** green baseline by checking the remaining standalone main error document collection. Inspect `ErrorCatalogDocument.Errors`; if it still dereferences the public mutable collection directly, add one red-first contract. This would complete the document-level collection family across the five standalone catalog validators.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorProfileCatalogValidatorNullProfilesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileCatalogDocument.Profiles` collection now becomes `ProfileCatalogProfilesCollectionIsNull` at `profiles` instead of throwing `NullReferenceException` on the first `.Count` dereference.