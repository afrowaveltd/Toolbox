...
`ErrorCatalogCrossValidator` now converts runtime-null document-level `Errors`, `Owners`, `CodeGroups`, `Categories`, and `Profiles` collections into stable validation issues before direct dereferences. It also converts null entries in those collections into stable null-definition issues. Nested cross-validator protection currently includes `ErrorDefinition.Categories`, `ErrorOwnerDefinition.Aliases`, `ErrorCategoryDefinition.Aliases`, `ErrorProfileDefinition.IncludeErrors`, `ErrorProfileDefinition.ExcludeErrors`, `ErrorProfileDefinition.IncludeOwners`, `ErrorProfileDefinition.IncludeCodeGroups`, and `ErrorProfileDefinition.IncludeCategories`. The owner/category remains indexable by its primary `Name` while malformed aliases are skipped, and malformed profile collections audited by the cross-validator no longer block independent profile checks. The standalone validators retain their already verified collection-null contracts for errors, owners, code groups, categories, profiles, and their audited nested mutable collections.
...
Continue the runtime/public-API audit from the **872-test** green baseline by moving beyond the now-audited nested mutable collections that `ErrorCatalogCrossValidator` actually dereferences. Do not add mechanical cross-validator tests for `IncludeSubcategories`, `IncludeTags`, or `ExcludeTags` unless the production cross-validator begins consuming them. Inspect the next runtime component for a real malformed mutable-collection dereference and add one red-first public contract around the first proven failure path.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullProfileIncludeCategoriesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.IncludeCategories` collection now becomes `ProfileIncludeCategoriesCollectionIsNull` at `profiles[index].includeCategories`; the null guard runs before the empty-index shortcut, and include-category validation is skipped safely while the remaining independent profile cross-validation continues. Production commit: `07df39bacf298deacf4d1f8d23d7cf8beea86cd9`. Red-first test commit: `c79902b63ddbe3b0e02c3530f7388f1effa023f4`.
