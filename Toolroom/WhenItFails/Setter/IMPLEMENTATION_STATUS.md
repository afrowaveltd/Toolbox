...
Continue the runtime/public-API audit from the **872-test** green baseline with the remaining semantically parallel nested code-group collection. Inspect `ErrorCodeGroupDefinition.DefaultTags`; it is passed directly into shared string collection validation and should receive its own red-first contract if no equivalent test already exists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCodeGroupCatalogValidatorNullDefaultCategoriesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCodeGroupDefinition.DefaultCategories` collection now becomes `CodeGroupDefaultCategoriesCollectionIsNull` at `codeGroups[index].defaultCategories` instead of throwing inside shared string collection validation.