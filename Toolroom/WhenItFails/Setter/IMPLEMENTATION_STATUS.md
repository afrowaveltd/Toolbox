...
Continue the runtime/public-API audit from the **872-test** green baseline through adjacent malformed-diagnostics boundaries. `CatalogProviderPipeline` now handles both null diagnostics collections and null issue entries. Inspect other helpers that extract the first dependency issue (`ErrorCatalogProvider`, `ErrorDescriptorResolver`, `ErrorCatalogInitializer`, runtime forwarding) and add a red-first contract only where a direct issue dereference can still escape as an exception or unstable response.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`CatalogProviderPipelineNullFirstIssueContractTests` is user-verified green: **1 passed**. A failed loader response whose mutable `Issues` collection contains a runtime-null first element no longer throws in `CatalogProviderPipeline.GetFirstIssueCode`; the pipeline now skips null issue entries and falls back to the caller-supplied code/message when no usable diagnostic exists. Production commit: `a1c8fa9d40440b1731d8b897ac120d31976fcdf7`. Red-first test commit: `8fd966c571096133fe9946f07d13b34085038c43`.
