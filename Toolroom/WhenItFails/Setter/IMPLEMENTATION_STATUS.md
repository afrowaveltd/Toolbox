...
Continue the runtime/public-API audit from the **872-test** green baseline by inspecting the remaining mutable nested collections in standalone validators for a real direct dereference. The category `DefaultTags` boundary is now protected. Do not add guards mechanically for `DefaultMappings` or `Metadata`; first prove an actual public failure path in current production.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCategoryCatalogValidatorNullDefaultTagsCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorCategoryDefinition.DefaultTags` collection now becomes `CategoryDefaultTagsCollectionIsNull` at `categories[index].defaultTags` instead of throwing inside shared string collection validation.