...
Continue the runtime/public-API audit from the **872-test** green baseline beyond `ErrorCatalogRuntime` and `ErrorDescriptorResolver`. Inspect the next resolver/provider composition boundaries for dereferences or enumerations of publicly mutable dependency-result collections, especially `Response<T>.Issues`, and add a red-first contract only where the current runtime path can actually throw.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorDescriptorResolverNullIssuesCollectionContractTests` is user-verified green: **1 passed**. A failed `IErrorDefinitionResolver` response with `Issues = null` no longer throws from `ErrorDescriptorResolver.GetFirstIssueCode`; `CreateById` returns the stable fallback failure code `ErrorDefinitionResolveFailed` and fallback message instead. Production commit: `3e0d0fa4da969d25e0762ba07daf26f9e23c3066`. Red-first test commit: `1906bf1a716122386cd363c109b3c36e5613af32`.
