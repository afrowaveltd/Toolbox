...
Continue the runtime/public-API audit from the **872-test** green baseline with the next proven nested mutable collection boundary in `ErrorCatalogCrossValidator`. `ValidateProfileIncludeCodeGroups` dereferences `ErrorProfileDefinition.IncludeCodeGroups` through `.Count` when the code-group index is non-empty, while the standalone profile validator already exposes the stable `ProfileIncludeCodeGroupsCollectionIsNull` contract. Add one red-first cross-validator contract with a non-empty supporting code-group catalog if no equivalent test exists.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullProfileIncludeOwnersCollectionContractTests` is user-verified green: **1 passed**. A runtime-null `ErrorProfileDefinition.IncludeOwners` collection now becomes `ProfileIncludeOwnersCollectionIsNull` at `profiles[index].includeOwners`; the null guard runs before the empty-index shortcut, and include-owner validation is skipped safely while the remaining independent profile cross-validation continues.
