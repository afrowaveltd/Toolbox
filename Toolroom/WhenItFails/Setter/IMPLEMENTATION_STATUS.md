...
- `ErrorProfileCatalogValidatorNullIncludeCodeGroupsCollectionContractTests`: **1 passed**;
- `ErrorProfileCatalogValidatorNullIncludeCategoriesCollectionContractTests`: **1 passed**;
- `ErrorProfileCatalogValidatorNullIncludeSubcategoriesCollectionContractTests`: **1 passed**;
- `ErrorProfileCatalogValidatorNullIncludeTagsCollectionContractTests`: **1 passed**;
- `ErrorProfileCatalogValidatorNullExcludeTagsCollectionContractTests`: **1 passed**;
- `ErrorCategoryCatalogValidatorNullDefaultTagsCollectionContractTests`: **1 passed**;
...
Continue the runtime/public-API audit from the **872-test** green baseline by reassessing nested mutable collection dereferences in `ErrorCatalogCrossValidator` and related runtime consumers. The five document-level collection boundaries consumed directly by the cross-validator are now protected and user-verified. Do not extend the series mechanically; prove the next public failure path in current production before adding a contract.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorCatalogCrossValidatorNullProfilesCollectionContractTests` is user-verified green: **1 passed**. A runtime-null supporting `ErrorProfileCatalogDocument.Profiles` collection now becomes `ProfileCatalogProfilesCollectionIsNull` at `profileCatalog.profiles`; both the initial null-entry scan and later profile cross-validation safely skip that malformed supporting collection instead of throwing.