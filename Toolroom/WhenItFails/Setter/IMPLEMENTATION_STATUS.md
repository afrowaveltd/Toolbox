...
The `AFW_THM_0012` slice is complete. The runtime/public-API audit now protects bootstrap DTOs, validation contracts, stable enum values, descriptor and definition models, provider payloads, catalog context and initialization payloads, `CatalogProviderPipeline`, `ErrorCatalog`, `ErrorCatalogFactory`, `ErrorDefinitionResolver`, `ErrorDescriptorFactory`, `ErrorDescriptorResolver`, `ErrorDescriptorService`, `ErrorCatalogRuntime`, `ErrorProfileSelectionService`, `ErrorCatalogContextProvider`, `ErrorCatalogProvider`, `BuiltInErrorCatalogContextProvider`, `ErrorCatalogInitializer`, `JsonsBootstrapper`, and `ErrorProfileResolver` public collection and malformed dependency-result boundaries. The shared catalog provider pipeline rejects runtime-null loader responses, normalizer results, validator results, and payload-factory results, protecting the category, code-group, owner, and profile providers through one common contract. `ErrorProfileResolver` normalization is directly contract-tested across separator, whitespace, casing, include, and exclusion/veto paths, and its resolved collection is genuinely read-only.
...
Continue the runtime/public-API audit from the new **872-test** green baseline. Prefer remaining public composition boundaries with meaningful status, validation, or read-only semantics; avoid duplicating loader basics, context-store invariants, or synthetic null permutations already structurally protected upstream.

Prefer one narrow contract with a clear public response shape.

## Last completed change

The complete `WhenItFails.Tests` regression run is user-verified green at **872 passed, 0 failed, 0 skipped**. Before that final run, the five historical `ErrorCatalogContextProviderNullResponseTaskResultTests` were updated from expecting `NullReferenceException` to asserting the current provider-specific stable `Invalid` response contract.