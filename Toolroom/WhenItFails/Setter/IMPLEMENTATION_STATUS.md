...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining bootstrap/template boundaries. Inspect `JsonsBootstrapper` template path/name handling for malformed dependency-returned values that can currently be interpreted as filesystem targets or leak infrastructure-shaped failures, and add a red-first contract only for a proven boundary. Then continue to adjacent composition helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonsBootstrapperNullTemplateTargetFileNameContractTests` is user-verified green: **1 passed**. A template returned by `IJsonsTemplateProvider` with runtime-null `TargetFileName` no longer reaches filesystem handling and degrades into a generic failure; `JsonsBootstrapper.EnsureWorkspaceAsync` now returns stable Invalid response `WIF_JSONS_TEMPLATE_TARGET_FILE_NAME_NULL` with message `The JSON template provider returned a template with a null target file name.` Production commit: `4f84176e2fdd543193b450126de7e6b42fc9f8b4`. Red-first test commit: `60027051df02628341ad7f159b89c1200c571cb0`.
