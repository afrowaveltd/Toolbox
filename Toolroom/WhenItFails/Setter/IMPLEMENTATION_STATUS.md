...
Continue the runtime/public-API audit from the **872-test** green baseline beyond the now-protected `ErrorProfileSelectionService` profile collection/entry boundaries. Inspect the next runtime component for a proven malformed mutable-collection dereference and add one red-first public contract around the first real failure path; do not add mechanical tests for fields that production does not consume.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`ErrorProfileSelectionServiceNullProfileDefinitionContractTests` is user-verified green: **1 passed**. A null `ErrorProfileDefinition` entry inside a non-null profile collection now returns an Invalid response with code `ErrorProfileDefinitionIsNull` instead of throwing while the profile-name predicate reads `candidate.Name` or `candidate.DisplayName`. Production commit: `96e44471977acbc7de75bc571da61ad0157d0e21`. Red-first test commit: `c4287092642a2c082789e5291a309f63a84daa3c`.
