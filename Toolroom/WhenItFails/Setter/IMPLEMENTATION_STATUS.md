...
Continue the runtime/public-API audit from the **872-test** green baseline across the remaining bootstrap/template path boundaries. Inspect `JsonsBootstrapper` for dependency-returned target names that can escape or redirect writes outside the intended package directory, and add a red-first contract only for a proven path boundary. Then continue to adjacent composition helpers.

Prefer one narrow contract with a clear public response shape.

## Last completed change

`JsonsBootstrapperWhitespaceTemplateTargetFileNameContractTests` is user-verified green: **1 passed**. A template returned by `IJsonsTemplateProvider` with an empty or whitespace `TargetFileName` no longer reaches filesystem handling and degrades into a generic failure; `JsonsBootstrapper.EnsureWorkspaceAsync` now returns stable Invalid response `WIF_JSONS_TEMPLATE_TARGET_FILE_NAME_EMPTY` with message `The JSON template provider returned a template with an empty target file name.` Production commit: `280d1218ff766a60b5b40469d5dcbfacc6f386f4`. Red-first test commit: `ff9c421fdfbb8fc541f9b48e33898c5858c0e1bd`.
