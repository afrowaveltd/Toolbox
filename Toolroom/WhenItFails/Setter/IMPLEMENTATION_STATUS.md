...
Continue the runtime/public-API audit with `ErrorCodeGroupCatalogDocument`.

Add focused coverage for its schema/language defaults and verify that tags, metadata, and code-group-definition collections are independently allocated between document instances. Keep the test namespace under `DefinitionContracts` to avoid shadowing the production `Definitions` namespace.
...
`ErrorCodeGroupDefinitionContractTests` passed all **2 focused tests**. Code-group definitions now have direct coverage for safe scalar defaults and independently allocated default categories, default tags, mappings, and metadata.