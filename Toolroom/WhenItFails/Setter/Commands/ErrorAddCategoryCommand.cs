...
internal static class ErrorAddCategoryCommand
{
    private const string Usage =
        "error-add-category <path> <id|code|name> <category-name|alias> [--json]";
    ...
    private static void ShowInvalidArguments()
    {
        CommandInputError.Show(
            "InvalidErrorAddCategoryArguments",
            "The error-add-category command requires a path, error lookup, category name or alias, and an optional --json switch.",
            Usage);
    }
    ...
    private static void ShowJsonFailure(Response<ErrorDefinition> response)
    {
        string failureCode = response.Issues.Count > 0
            ...
            : "ErrorAddCategoryFailed";
        string failureMessage = string.IsNullOrWhiteSpace(response.Message)
            ? "The category could not be added to the error definition."
            : response.Message;
    ...
    private sealed record ErrorAddCategoryResult(
        bool Updated,
